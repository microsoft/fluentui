...
By default `merge-styles` will initially inject a `style` element into the document head as its first node and then append after the last one added. When a specific position of the initial `style` element in the document head is desired, you can place a placeholder `style` element in the head with `data-merge-styles` attribute on it and it will serve as reference point from where to start appending the next `style` elements generated by `merge-styles` (Ex: `<style data-merge-styles></style>`).
