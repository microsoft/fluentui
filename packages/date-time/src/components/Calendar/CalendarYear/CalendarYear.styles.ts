...
export const getStyles = (props: ICalendarYearStyleProps): ICalendarYearStyles => {
  const { className, theme, hasHeaderClickCallback, highlightCurrentYear, highlightSelectedYear } = props;
  const { palette } = theme;
  ...
  return {
    ...
    currentYear: highlightCurrentYear
      ? {
          color: palette.white,
      ...
      : {},
    selectedYear: highlightSelectedYear
      ? {
          color: palette.neutralPrimary,
          backgroundColor: palette.themeLight,
          fontWeight: FontWeights.semibold,
