...
function loadSite() {
  ...
  const isProduction = getParameterByName('prod') === '1' || getParameterByName('isProduction') === '1';
  const siteVariant = getSiteVariant();

  if (siteVariant === 'prDeploy' || siteVariant === 'local' || !siteVariant) {
...
function getSiteVariant() {
  ...
  let variant: keyof typeof expectedHosts;
