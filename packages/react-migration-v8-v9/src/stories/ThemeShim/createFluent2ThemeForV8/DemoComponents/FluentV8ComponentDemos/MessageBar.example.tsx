import type { IChoiceGroupStyles, IStackProps } from '@fluentui/react';
import {
  ChoiceGroup,
  Link,
  MessageBar,
...
const FullWidthExample = (p: IExampleProps) => (
  <MessageBar
    messageBarType={MessageBarType.warning}
    isMultiline={false}
    onDismiss={p.resetChoice}
    dismissButtonAriaLabel="Close"
    styles={fullWidthMessageBarStyle}
    actions={
      <div>
        <MessageBarButton>Action</MessageBarButton>
      </div>
  ...
  >
    Full Width ( Squared Borders ) Warning MessageBar content.
    <Link href="www.bing.com" target="_blank" underline>
      Visit our website.
    </Link>
  </MessageBar>
...
const choiceOptions = [
  ...
  { key: 'fullWidth', text: 'Warning - full width ( squared borders )' },
];
    ...
    <div>
      ...
      <Stack {...horizontalStackProps}>
        ...
        <Stack {...verticalStackProps}>
          ...
          {(choice === 'fullWidth' || showAll) && <FullWidthExample resetChoice={resetChoice} />}
        </Stack>
      </Stack>
    </div>
  );
