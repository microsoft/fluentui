 ...
 * having to manually maintain a list.
 *
 ...
 */
const definitions = import.meta.glob('../../src/*/define-async.{ts,js}');

await Promise.all(Object.values(definitions).map(loadDefinition => loadDefinition()));
