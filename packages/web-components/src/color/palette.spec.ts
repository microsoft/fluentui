...
describe('palette', (): void => {
  ...
  it('should return upper-case hex values', (): void => {
    (palette(PaletteType.neutral)(DesignSystemDefaults) as Palette).forEach((swatch: Swatch) => {
      expect(swatch).to.equal(swatch.toUpperCase());
    });
    (palette(PaletteType.accent)(DesignSystemDefaults) as Palette).forEach((swatch: Swatch) => {
      expect(swatch).to.equal(swatch.toUpperCase());
    });
  });
...
describe('findSwatchIndex', (): void => {
  it('should implement design-system defaults', (): void => {
    ...
    expect(findSwatchIndex(accentPalette, accentBaseColor({} as DesignSystem))({} as DesignSystem)).to.equal(59);
  });
  ...
  it('should find white', (): void => {
    ...
    expect(findSwatchIndex(neutralPalette, 'rgb(255, 255, 255)')(DesignSystemDefaults)).to.equal(0);
  });
  ...
  it('should find black', (): void => {
    ...
    expect(findSwatchIndex(neutralPalette, 'rgb(0, 0, 0)')(DesignSystemDefaults)).to.equal(93);
  });

  it('should find accent', (): void => {
    expect(findSwatchIndex(accentPalette, accentBaseColor(DesignSystemDefaults))(DesignSystemDefaults)).to.equal(59);
    expect(findSwatchIndex(accentPalette, 'rgb(0, 120, 212)')(DesignSystemDefaults)).to.equal(59);
  });
});

describe('findClosestSwatchIndex', (): void => {
  it('should return 0 if the input swatch cannot be converted to a color', (): void => {
    expect(findClosestSwatchIndex(neutralPalette, 'pewpewpew')({} as DesignSystem)).to.equal(0);
  });
  it('should operate on design system defaults', (): void => {
    expect(findClosestSwatchIndex(neutralPalette, '#FFFFFF')({} as DesignSystem)).to.equal(0);
    expect(findClosestSwatchIndex(neutralPalette, '#808080')({} as DesignSystem)).to.equal(49);
    expect(findClosestSwatchIndex(neutralPalette, '#000000')({} as DesignSystem)).to.equal(93);
  });
  it('should return the index with the closest luminance to the input swatch if the swatch is not in the palette', (): void => {
    expect(findClosestSwatchIndex(neutralPalette, '#008000')({} as DesignSystem)).to.equal(56);
    expect(findClosestSwatchIndex(neutralPalette, '#F589FF')({} as DesignSystem)).to.equal(30);
  });
});
...
describe('swatchByMode', (): void => {
  it('should operate on DesignSystemDefaults', (): void => {
    expect(swatchByMode(neutralPalette)(0, 0)({} as DesignSystem)).to.equal(DesignSystemDefaults.neutralPalette[0]);
    expect(swatchByMode(accentPalette)(0, 0)({} as DesignSystem)).to.equal(DesignSystemDefaults.accentPalette[0]);
  });
...
describe('swatchByContrast', (): void => {
  ...
  describe('indexResolver', (): void => {
    it('should pass a reference color as the first argument', (): void => {
      ...
      swatchByContrast('#FFF')(neutralPalette)(indexResolver as any)(directionResolver as any)(
        contrastCondition as any,
      )({} as DesignSystem);
    ...
    it('should pass the palette as the second argument', (): void => {
      ...
      swatchByContrast('#FFF')(() => colorPalette)(indexResolver as any)(directionResolver as any)(
        contrastCondition as any,
      )({} as DesignSystem);
      expect(indexResolver).to.have.been.called.once;
  ...
  describe('directionResolver', (): void => {
    ...
    it('should receive the palette length - 1 if the resolved index is greater than the palette length', (): void => {
      ...
      expect(directionResolver).to.have.been.called.once;
      expect(directionResolver).to.have.been.called.with(neutralPalette({} as DesignSystem).length - 1);
    });
  ...
  it('should return the color at the initial index if it satisfies the predicate', (): void => {
    ...
    expect(
      swatchByContrast('#FFF')(() => sourcePalette)(indexResolver)(directionResolver)(contrastCondition)(designSystem),
