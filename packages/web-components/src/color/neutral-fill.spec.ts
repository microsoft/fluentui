...
describe('neutralFill', (): void => {
  ...
  it('should operate on design system defaults', (): void => {
    [neutralFillActive, neutralFillFocus, neutralFillHover, neutralFillRest, neutralFillSelected].forEach(fn => {
      expect(neutralPalette).to.include(fn({} as DesignSystem));
    });
  });

  it('should switch from dark to light after 10 swatches', (): void => {
    expect(neutralFillRest(DesignSystemDefaults)).to.equal(neutralPalette[DesignSystemDefaults.neutralFillRestDelta]);
    expect(neutralFillHover(DesignSystemDefaults)).to.equal(neutralPalette[DesignSystemDefaults.neutralFillHoverDelta]);
    expect(neutralFillActive(DesignSystemDefaults)).to.equal(
      neutralPalette[DesignSystemDefaults.neutralFillActiveDelta],
    );
    expect(neutralFillFocus(DesignSystemDefaults)).to.equal(neutralPalette[DesignSystemDefaults.neutralFillFocusDelta]);
    ...
    expect(neutralFillRest(() => neutralPalette[10])(DesignSystemDefaults)).to.equal(neutralPalette[3]);
  });
  ...
  it('should have consistent return values', (): void => {
    neutralPalette.concat(accentPalette).forEach((swatch: Swatch): void => {
      const backplates: FillSwatchFamily = neutralFill(() => swatch)(DesignSystemDefaults);
...
describe('ensure parity between old and new recipe implementation', () => {
  ...
  palette.swatches.forEach((newSwatch, index) => {
    ...
    const oldValues = neutralFill({
      ...DesignSystemDefaults,
      backgroundColor: DesignSystemDefaults.neutralPalette[index],
    });
    const newValues = neutralFillNew(
      palette,
      newSwatch,
      neutralFillRestDelta,
      neutralFillHoverDelta,
