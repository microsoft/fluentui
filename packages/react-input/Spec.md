...
- `type` by default has type `string`, but it's restricted to a list of only the text-like values [listed on MDN](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/Input#input_types). I think it's worth making this explicit to avoid people thinking that Input can handle all the same behaviors as a native `<input>`.
