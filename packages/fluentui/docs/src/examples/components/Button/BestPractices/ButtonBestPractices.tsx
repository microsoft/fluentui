...
const doList = [
  ...
  <Text>
    When button is in loading state, use {code('disabledFocusable')} attribute to ensure best experience for screen
    reader, then {link('buttonBehavior', '/components/button/accessibility#default-behavior')} adds{' '}
    {code('aria-disabled')} attribute. When {code('disabled')} attribute is used, then button cannot receive focus.
  </Text>,
];
