...
### Using media, container, and feature queries

The `Flex` component supports CSS at-rules such as `@media`, `@container`, and `@supports` within the `styles` prop. This allows you to create responsive layouts that adapt based on viewport size, container dimensions, or browser feature support. In the example below, the box changes its background color depending on which conditions are met — try resizing your browser window or the container to see the different states.

<ExampleSnippet>
  <Flex style={{width:'100%', containerType: 'inline-size', containerName:'sidebar' }}>
      <Flex
        styles={{
          width: '100%',
          minHeight: '200px',
