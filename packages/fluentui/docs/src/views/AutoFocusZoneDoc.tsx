...
export default () => (
  <DocPage title="Auto Focus Zone">
    ...
    <p>
      {code('AutoFocusZone')} can be integrated into {code('Popup')} component and is used to grab focus and put it to an inner element when
      the component mounts. This can be achieved by specifying {code('autoFocus')} prop on the {code('Popup')}.
    </p>
    <p>
      If you need both - grabbing the focus and trap the focus in the component - use {code('trapFocus')} prop for {code('Popup')}
      to integrate <Link to="focus-trap-zone">FocusTrapZone</Link>.
    </p>
    <Header as="h2">Usage</Header>
    <p>
      In {code('Popup')}, set {code('autoFocus')} to {code('true')} with default settings or set an object with desired values for auto
      focus zone props.
    </p>
    ...
    <p>
      For example, we want to specify the focusable selector for Popup with auto focus. On Popup mount, focus will go to the element matched
      to that selector. For that purpose, we can specify {code('firstFocusableSelector')}.
    </p>
    <CodeSnippet
