...
export default () => (
  <DocPage title="Performance">
    ...
    <p>
      To avoid unnecessary re-rendering of components that use {link('Shorthand API', '/shorthand-prop')}, always gather the required data
      before rendering of the component, potentially by {link('lifting the state up', 'https://reactjs.org/docs/lifting-state-up.html')}.
    </p>
    <p>
      For container components, especially if rendering of the items might be an expensive operation, memoization (for example{' '}
      {code('React.memo')}) of items can be used:
      <ExampleSnippet
        value={`
