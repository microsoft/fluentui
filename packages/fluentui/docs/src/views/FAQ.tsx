...
export default () => (
  <DocPage title="FAQ">
    ...
    <ul>
      <li>
        ...
        <ul>
          ...
          <li>{link("How can @mixins be used in the Fluent UI's styles?", '#how-can-mixins-be-used-in-the-fluent-uis-styles')}</li>
    ...
    <Answer
      content={
        <p>
          As the styles in Fluent UI are defined using CSS in JS, mixins can be defined as simple functions that can be reused on multiple
          places.
          <br />
    ...
    <Answer
      content={
        <p>
          Just as with vanilla React, all HTML props are supported on all Fluent UI components. Just pass onLoad or onError to the component
          you want to put the prop on.
          <CodeSnippet
            value={`
    ...
    <Answer
      content={
        <div>
          <p>
            {code('Flex')}, {code('Grid')} and {code('Layout')} components handle layout aspects. The {code('Flex')} component is for laying
            out items in one direction, while the {code('Grid')} component is made for two dimensional layouts. Visit the{' '}
            {link('Layout guide', '/layout')} page for a detailed comparison between {code('Flex')}
            and {code('Grid')}.
          </p>
          <p>
            The {code('Layout')} component is now deprecated. Its purpose was arrangement of the content of a component. {code('Flex')} or{' '}
            {code('Grid')} component should be used instead of the {code('Layout')}
            component.
          </p>
          <p>
            {code('Box')} is a utility component that is often used by Fluent UI to implement higher-level components. By default, it
            renders styled {code('div')} element.
          </p>
          <p>
            The cases when client might want to use it are very exceptional, and all are about applying custom styles to rendered{' '}
            {code('div')} element. However, more robust approach that won't break theming consistency is to instead create a custom
            component and define related styles as part of a theme. Visit{' '}
            {link('Integrate Custom Components', '/integrate-custom-components')}
            page to see how this can be done.
          </p>
          <p>{link('Segment', '/components/segment')} groups related content together. It shouldn't be used to handle layout aspects.</p>
        </div>
      }
    />
    ...
    <Answer
      content={
        <p>
          Generally Teams should be using SVG icons only. If you need FontAwesome or other icons, process for adding them is described in{' '}
      ...
      content={
        <>
          ...
          <CodeSnippet
            mode="js"
            value={`
              import { themes } from '@fluentui/react-experimental'
    ...
    />
    <GuidesNavigationFooter previous={{ name: 'Quick Start', url: 'quick-start' }} next={{ name: 'Accessibility', url: 'accessibility' }} />
  </DocPage>
);
