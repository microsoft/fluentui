...
Fluent UI provides tools for developers to debug styles and measure performance.

## Debugging styles

### How merging works

As it was described in [Theming](/theming), a theme consists of three different building blocks:

1. Site variables,
2. Component variables,
3. Component styles (for individual component slots).

Themes can be nested, component variables and styles can be overriden in a component instance:
...
4. Merges component styles for each component slot are passed to CSSInJS renderer to process the styles with plugins (RTL, expand, vendor prefixing), generate CSS classes and add the styles to DOM.

As the merged component styles are processes by CSSInJS and the way how those are added to DOM is CSSInJS specific (Fela uses atomic CSS classes adding a class for each CSS attribute) it might be difficult to debug the styles using browser's development tools.
...
Both Browser and React DevTools provide developers with great tools to measure performance. FluentUI adds its own Telemetry popover to give developers
...
In the table you can see times spent rendering individual FluentUI components since the telemetry was enabled (or application loaded).
...
**Total** shows the total time in ms spent in FluentUI rendering the component. You can also expand this column to show minimum (**Min**), maximum (**Max**) and average (**Avg**) time of individual renders.

Most expensive part of FluentUI is the styles computation - this time is in **Styles** column. As it was described in [How merging works](#how-merging-works),
...
To improve performance, FluentUI caches the styles and classes. **Cache hits** column shows how effective the cache is for individual component - for its root slot (**root**) as well as for the other slots (**slots**).
