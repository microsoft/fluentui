...
import { getCallbackTypeNode } from './getCallbackTypeNode';
...
export function getCallbackArguments(
  ...
  const callbackTypeNode = getCallbackTypeNode(program, filename, typeName, propertyName);

  // Handles a case when a type is defined by reference
  //
  // export type Callback = () => {}
  // export type ComponentProps = { onClick: Callback }
  if (ts.isTypeReferenceNode(callbackTypeNode)) {
    const typeAtLocation = typeChecker.getTypeAtLocation(callbackTypeNode);
  ...
  return parseFunctionArguments(typeChecker, callbackTypeNode);
}
