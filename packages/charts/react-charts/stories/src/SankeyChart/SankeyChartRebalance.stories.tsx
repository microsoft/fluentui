...
export const SankeyChartRebalance: React.FunctionComponent<{}> = (): JSXElement => {
  ...
  const _onDataSourceChange = React.useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    setDataSource(e.currentTarget.checked ? DataSouce.Simple : DataSouce.Complex);
  }, []);

  const data: ChartProps = dataSource === DataSouce.Simple ? dataSimple : dataComplex;
