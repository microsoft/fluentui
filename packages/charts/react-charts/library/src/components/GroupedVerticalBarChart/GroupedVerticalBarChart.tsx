...
>((props = { maxBarWidth: 24 }, forwardedRef) => {
  ...
  const _adjustProps = () => {
    ...
    _xAxisInnerPadding = getScalePadding(
      ...
      undefined,
      2 / (2 + _legends.length + (_legends.length - 1) * BAR_GAP_RATE),
    );
  ...
  const _getLegendData = (): JSX.Element => {
  ...
  const legendsElement: JSX.Element = _getLegendData();
  _adjustProps();
  ...
  const _getGraphData = (
    ...
    _barWidth = getBarWidth(
      props.barWidth,
      props.maxBarWidth,
      xScale0.bandwidth() / (_legends.length + (_legends.length - 1) * BAR_GAP_RATE),
    );
    _groupWidth = (_legends.length + (_legends.length - 1) * BAR_GAP_RATE) * _barWidth;
  ...
  const _buildGraph = (
    ...
    _legends.forEach((legendTitle: string, legendIndex: number) => {
      ...
      if (barPoints) {
        ...
        barPoints.forEach((pointData: GVBarChartSeriesPoint, pointIndex: number) => {
          ...
          singleGroup.push(
            <rect
              ...
              onFocus={event => onBarFocus(event, pointData, singleSet, legendIndex)}
              onBlur={_onBarLeave}
              onClick={pointData.onClick}
              aria-label={getAriaLabel(pointData, singleSet.xAxisPoint)}
              tabIndex={isLegendActive ? 0 : undefined}
              role="img"
            />,
          );
        ...
        if (barTotalValue && !props.hideLabels && _barWidth >= 16 && isLegendActive) {
  ...
  const _getDomainMargins = (containerWidth: number): Margins => {
    ...
    if (_xAxisType === XAxisTypes.StringAxis) {
      if (isScalePaddingDefined(props.xAxisOuterPadding)) {
        ...
        _barWidth = getBarWidth(props.barWidth, props.maxBarWidth);
        const groupWidth = (_legends.length + (_legends.length - 1) * BAR_GAP_RATE) * _barWidth;
        ...
        const barBandwidth = groupBandwidth / (_legends.length + (_legends.length - 1) * BAR_GAP_RATE);
        const barWidth = getBarWidth(props.barWidth, props.maxBarWidth, barBandwidth);
        const groupWidth = (_legends.length + (_legends.length - 1) * BAR_GAP_RATE) * barWidth;
