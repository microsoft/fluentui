...
import { IKeySequence, convertSequencesToKeytipID, ktpLayerId } from '../../Utilities';
...
describe('getAriaDescribedBy', () => {
  ...
  it('for one singular key sequence', () => {
    ...
    const ariaDescribedBy = getAriaDescribedBy(keySequence);
    expect(ariaDescribedBy).toEqual(ktpLayerId + ' ' + convertSequencesToKeytipID(keySequence));
  });
