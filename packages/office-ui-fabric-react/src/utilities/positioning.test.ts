...
function positionCalloutTest(testValues: ITestValues, alignment: DirectionalHint, validate: ITestValidation) {
  ...
  let result: positioningFunctions.ICallout = positioningFunctions._positionCalloutWithinBounds(
    callout,
    target,
    bounds,
    positioningFunctions._getPositionData(alignment, target, bounds),
    gap
  );
  ...
  assert(
    result.calloutRectangle.equals(validate.callout),
    'Callout not alligned with target ' + stringifyResults(validate.callout, result.calloutRectangle)
  );

  assert(
    beak.equals(validate.beak as Rectangle),
    'Beak is improperly positioned. ' + stringifyResults(validate.beak, beak)
  );
}
...
function validateNoBeakTest(testValues: ITestValues, alignment: DirectionalHint, validate: ITestValidation) {
  ...
  assert(
    result.calloutRectangle.equals(validate.callout),
    'No Beak: Callout not alligned with target ' + stringifyResults(validate.callout, result.calloutRectangle)
  );
}
...
describe('Callout Positioning', () => {
  ...
  it('Correctly recalculates percents', () => {
    ...
    let result = positioningFunctions._recalculateMatchingPercents(
      new Rectangle(0, 100, 300, 400),
      RectangleEdge.top,
      targetRectangle,
    ...
    result = positioningFunctions._recalculateMatchingPercents(
      new Rectangle(200, 300, 300, 400),
      RectangleEdge.top,
      targetRectangle,
      RectangleEdge.bottom,
      50
  ...
  it('Correctly positions the callout with the beak', () => {
    ...
    let validateBottomLeft: ITestValidation = {
      callout: new Rectangle(
        400,
        700,
        800 + positioningFunctions._calculateActualBeakWidthInPixels(8),
        1100 + positioningFunctions._calculateActualBeakWidthInPixels(8)
      ),
  ...
  it('Correctly determines max height', () => {
    ...
    assert(
      testMax === 1000 - targetBot,
      `Test for maxHeight from bottom of target to bottom of bounds: maxHeight was ${testMax} and it should have been ${1000 -
        targetBot}`
    );
    ...
    assert(
      testMax === 1000 - targetTop,
      `Test for maxHeight from top of target to bottom of bounds: maxHeight was ${testMax} and it should have been ${1000 -
        targetTop}`
    );
  });
  ...
  it('Correctly determines max height with a gapSpace included', () => {
    ...
    assert(
      testMax === 1000 - targetBot - gapSpace,
      `Test for maxHeight from bottom of target to bottom of bounds: maxHeight was ${testMax} and it should have been ${1000 -
        targetBot -
        gapSpace}`
    );
    ...
    assert(
      testMax === 1000 - targetTop - gapSpace,
      `Test for maxHeight from top of target to bottom of bounds: maxHeight was ${testMax} and it should have been ${1000 -
        targetTop -
        gapSpace}`
    );
