...
import { BaseComponent, assign, buttonProperties, getNativeProps, KeyCodes } from '../../Utilities';
...
export class ContextualMenuSplitButton extends BaseComponent<IContextualMenuSplitButtonProps, IContextualMenuSplitButtonState> {
  ...
  public render(): JSX.Element | null {
    const { item, classNames, index, focusableElementIndex, totalItemCount, hasCheckmarks, hasIcons, onItemMouseLeave } = this.props;
  ...
  private _renderSplitIconButton(item: IContextualMenuItem, classNames: IMenuItemClassNames, index: number) {
    const { contextualMenuItemAs: ChildrenRenderer = ContextualMenuItem, onItemMouseLeave, onItemMouseDown } = this.props;
  ...
  private _executeItemClick = (ev: React.MouseEvent<HTMLElement> | React.KeyboardEvent<HTMLElement>): void => {
    const { item, executeItemClick } = this.props;
