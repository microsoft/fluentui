...
const KeyPrefix: string = 'fabricImage';
...
export class ImageBase extends BaseComponent<IImageProps, IImageState> {
  ...
  public render() {
    ...
    const imageProps = getNativeProps(this.props, imageProperties, ['width', 'height', 'className']);
    const { loadState } = this.state;
    const coverStyle = this.props.coverStyle !== undefined ? this.props.coverStyle : this._coverStyle;
    const isLoaded = loadState === ImageLoadState.loaded || (loadState === ImageLoadState.notLoaded && this.props.shouldStartVisible);
    const classNames = getClassNames(
      getStyles,
      {
        className,
        imageFit,
        isPortrait: this._coverStyle === ImageCoverStyle.portrait,
        loadState,
        maximizeFrame,
        width,
        height,
        shouldFadeIn: isLoaded && shouldFadeIn && !shouldStartVisible
      });
  ...
  private _checkImageLoaded(): void {
    ...
    if (loadState === ImageLoadState.notLoaded) {
      ...
      const isLoaded: boolean = src && this._imageElement && (this._imageElement.naturalWidth > 0 && this._imageElement.naturalHeight > 0) ||
        (this._imageElement.complete && SvgRegex.test(src!));
