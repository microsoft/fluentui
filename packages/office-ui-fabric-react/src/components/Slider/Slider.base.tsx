...
/**
* Slider with no default styles.
* [Use the `getStyles` API to add your own styles.](https://github.com/OfficeDev/office-ui-fabric-react/wiki/Styling)
*/
@customizable('Slider', ['theme'])
export class SliderBase extends BaseComponent<ISliderProps, ISliderState> implements ISlider {
  ...
  private _classNames: IClassNames<ISliderStyles>;
  ...
  public render(): JSX.Element {
    ...
    return (
      ...
      >
        { label && (
          /* @TODO(keco): Check this */
          <Label
            className={ styles.titleLabel }
            { ...ariaLabel ? {} : { 'htmlFor': this._id } }
          >
            { label }
          </Label>
        ) }
        ...
        >
          <button
            ...
            { ...onKeyDownProp }
            { ...buttonProps }
            className={ this._classNames.slideBox }
            id={ this._id }
            disabled={ disabled }
            type='button'
            role='slider'
          >
            ...
            >
              <span
                ref={ this._thumb }
                className={ this._classNames.thumb }
                /* @TODO(keco): Check this */
                style={ this._getThumbStyle(vertical, thumbOffsetPercent) }
              />
              <span
                className={ this._classNames.active }
                /* @TODO(keco): Check this */
                style={ { [lengthString]: thumbOffsetPercent + '%' } }
              />
              <span
                className={ this._classNames.inactive }
                /* @TODO(keco): Check this */
                style={ { [lengthString]: (100 - thumbOffsetPercent) + '%' } }
              />
            </div>
          </button>
          { showValue && <Label className={ this._classNames.value }>{ value }</Label> }
        </div>
      </div>
    );
  }
