...
Callouts that do not contain focusable elements (links, buttons etc) cannot gain focus using `setInitialFocus`. For proper screen reader support follow the `Non Focused Callout` example which treats the callout content like a status message.
