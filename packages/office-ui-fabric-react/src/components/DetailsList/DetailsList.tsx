...
import {
  ...
  SelectAllVisibility,
  IDetailsHeaderProps
} from "../DetailsList/DetailsHeader";
...
export class DetailsList
  extends BaseComponent<IDetailsListProps, IDetailsListState>
  ...
  constructor(props: IDetailsListProps) {
    ...
    this._onGroupExpandStateChanged = this._onGroupExpandStateChanged.bind(
      this
    );
    ...
    this._selection =
      props.selection ||
      new Selection({ onSelectionChanged: undefined, getKey: props.getKey });
              ...
              >
                {groups ? (
                  ...
                  <List
                    ...
                    role="presentation"
                    items={
                      enableShimmer && !items.length ? SHIMMER_ITEMS : items
                    }
  ...
  private _adjustColumns(
    ...
    const adjustedColumns = this._getAdjustedColumns(
      newProps,
      forceUpdate,
      resizingColumnIndex
    );
  ...
  private _getAdjustedColumns(
    ...
    const lastSelectionMode = this.state
    ...
    if (layoutMode === DetailsListLayoutMode.fixedColumns) {
      ...
      adjustedColumns.forEach(column => {
        this._getColumnOverride(column.key).currentWidth =
  ...
  private _getFixedColumns(newColumns: IColumn[]): IColumn[] {
    return newColumns.map(column => {
      const newColumn: IColumn = assign(
        {},
        column,
  ...
  private _getJustifiedColumnsAfterResize(
    ...
    fixedColumns.forEach(
      column =>
        (column.calculatedWidth = this._getColumnOverride(
          column.key
        ).currentWidth)
    );

    const fixedWidth = fixedColumns.reduce(
      (total, column, i) => total + getPaddedWidth(column, i === 0),
      0
    );
  ...
  private _getJustifiedColumns(
    ...
    const availableWidth =
      viewportWidth - (outerPadding + rowCheckWidth + groupExpandWidth);
  ...
  private _getColumnOverride(key: string): IColumn {
    return (this._columnOverrides[key] =
      this._columnOverrides[key] || ({} as IColumn));
  }
...
function getPaddedWidth(column: IColumn, isFirst: boolean): number {
  return (
    column.calculatedWidth! +
    (isFirst ? 0 : DEFAULT_INNER_PADDING) +
    (column.isPadded ? ISPADDED_WIDTH : 0)
  );
}
