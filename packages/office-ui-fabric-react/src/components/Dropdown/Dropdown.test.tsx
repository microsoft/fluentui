...
describe("Dropdown", () => {
  ...
  describe("single-select", () => {
    ...
    it("Can flip between enabled and disabled.", () => {
      ...
      ReactDOM.render(
        <Dropdown label="testgroup" options={DEFAULT_OPTIONS} />,
        container
      );
      const dropdownRoot = container.querySelector(
        ".ms-Dropdown"
      ) as HTMLElement;

      expect(dropdownRoot.className).not.toEqual(
        expect.stringMatching("is-disabled")
      );
      ...
      expect(dropdownRoot.className).toEqual(
        expect.stringMatching("is-disabled")
      );
    ...
    it("Renders no selected item in default case", () => {
      ...
      const titleElement = dropdownRoot.querySelector(
        ".ms-Dropdown-title"
      ) as HTMLElement;
    ...
    it("does clear when the selectedKey is null", () => {
      const wrapper = mount(
        <Dropdown selectedKey="1" options={DEFAULT_OPTIONS} />
      );
    ...
    it("Can change items in uncontrolled case", () => {
      ...
      try {
        ...
        const secondItemElement = document.querySelector(
          '.ms-Dropdown-item[data-index="2"]'
        ) as HTMLElement;
        ...
        expect(
          dropdownRoot!.querySelector(".ms-Dropdown-title")!.textContent
        ).toEqual("2");
      }
    });
    ...
    it("sets the selected item even when key is number 0", () => {
      ...
      const wrapper = mount(
        <Dropdown componentRef={dropdown} options={options} />
      );
    ...
    it("Keypresses on a disabled dropdown has no effect.", () => {
      ...
      ReactDOM.render(
        <Dropdown label="testgroup" disabled options={options} />,
        container
      );
      const dropdownRoot = container.querySelector(
        ".ms-Dropdown"
    ...
    it("Keypresses on a normal dropdown selects the right, valid items.", () => {
      ...
      options[3] = { key: 3, text: "3", selected: true };
      ReactDOM.render(
        <Dropdown label="testgroup" options={options} />,
        container
      );
      const dropdownRoot = container.querySelector(
        ".ms-Dropdown"
    ...
    it("Will skip over headers and separators on keypress", () => {
      ...
      ReactTestUtils.Simulate.keyDown(dropdownRoot, { which: KeyCodes.down });
      titleElement = dropdownRoot.querySelector(
        ".ms-Dropdown-title"
      ) as HTMLElement;
      expect(titleElement.textContent).toEqual("1");
    ...
    it("Shows correct tooltip with and without title prop specified", () => {
      ...
      const thirdItemElement = document.querySelector(
        '.ms-Dropdown-item[data-index="3"]'
      ) as HTMLElement;
  ...
  describe("multi-select", () => {
    it("Renders multiselect Dropdown correctly", () => {
      const component = renderer.create(
        <Dropdown options={DEFAULT_OPTIONS} multiSelect />
      );
      const tree = component.toJSON();
      expect(tree).toMatchSnapshot();
    });
    ...
    it("Renders no selected item in default case", () => {
      ...
      ReactDOM.render(
        <Dropdown label="testgroup" options={DEFAULT_OPTIONS} multiSelect />,
        container
      );
      const dropdownRoot = container.querySelector(
        ".ms-Dropdown"
    ...
    it("sets the selected items even when key is number 0", () => {
      ...
      const dropdown = React.createRef<IDropdown>();
      const wrapper = mount(
        <Dropdown multiSelect componentRef={dropdown} options={options} />
      );
  ...
  describe("Aria attributes", () => {
    it("does not apply aria-labelledby if no label is provided", () => {
      ...
      expect(
        dropdownRoot.attributes.getNamedItem("aria-labelledby")
      ).toBeNull();
    });
    ...
    it("applies aria-labelledby if a non-empty label is provided", () => {
      ...
      ReactDOM.render(
        <Dropdown label="Test label" options={options} />,
        container
      );
      const dropdownRoot = container.querySelector(
        ".ms-Dropdown"
  ...
  describe("with simulated async loaded options", () => {
    ...
    class DropdownWithChangingProps extends React.Component<
      { multi: boolean },
    ...
    it("defaultSelectedKey value is respected if Dropdown options change for single-select Dropdown.", () => {
      ...
      const dropdownOptionText = container.querySelector(
