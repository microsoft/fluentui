...
export class CommandBarCustomizationExample extends React.Component<{}, ISplitDropDownButtonState> {
  ...
  private _renderSplitButtonMenuItem = (item: IContextualMenuItem): JSX.Element => {
    ...
    const dropDownButtonClass = css(styles.button, darkerBG);
    const mainBtnClassName = css(!item.name && 'ms-CommandBarItem--noName', styles.button, darkerBG);

    return (
      <div>
        <div className={css(styles.customButtonContainer, darkerBG)} ref={ref => (this.container = ref!)}>
          <CommandButton className={mainBtnClassName} iconProps={{ iconName: 'Add' }} text="New" />
