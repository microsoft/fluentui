...
export class ThemeGenerator {
   ...
   * @param overwriteCustomColor - a slot could have a generated color based on its inheritance rules (isCustomized is false), or a custom
   * color based on user input (isCustomized is true). This bool tells us whether to override existing customized colors.
   */
  ...
  /** @deprecated Use `ensureSlots` */
  public static insureSlots(slotRules: IThemeRules, isInverted: boolean) {
    return ThemeGenerator.ensureSlots(slotRules, isInverted);
  }

  /**
   * Gets the JSON-formatted blob that describes the theme, usable with REST endpoints.
   *
   * ```
   * {
   ...
   * @param isCustomization - true means it's a user provided color--set it to that raw color.
   * false means the rule it's inheriting from changed, so updated using asShade.
   */
