...
const DEFAULT_OPTIONS4: IComboBoxOption[] = [
  { key: '1', text: 'One' },
  { key: '2', text: 'Foo', ariaLabel: 'Label' },
  { key: '3', text: 'Bar' }
];
...
describe('ComboBox', () => {
  ...
  it('Item aria label defaults to item text', () => {
    let comboBoxRoot;
    let buttonElement;

    wrapper = mount(<ComboBox label="testgroup" defaultSelectedKey="1" options={DEFAULT_OPTIONS4} />);
    comboBoxRoot = wrapper.find('.ms-ComboBox');
    buttonElement = comboBoxRoot.find('button');
    buttonElement.simulate('click');

    const secondItemElement: Element = wrapper.getDOMNode().ownerDocument.querySelector('.ms-ComboBox-option[data-index="0"]')!;

    expect(secondItemElement.getAttribute('aria-label')).toBe('One');
  });

  it('Item aria label uses ariaLabel if provided', () => {
    let comboBoxRoot;
    let buttonElement;
