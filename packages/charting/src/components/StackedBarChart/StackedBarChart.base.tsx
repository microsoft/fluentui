...
export class StackedBarChartBase extends React.Component<IStackedBarChartProps, IStackedBarChartState> {
  ...
  public render(): JSX.Element {
    this._adjustProps();
    const { data, hideNumberDisplay, hideLegend, theme, styles, barBackgroundColor, href, overrideFixStyle } = this.props;
    const { palette } = theme!;
    const barHeight = overrideFixStyle || data!.chartData!.length > 2 ? this.props.barHeight : 8;
    const bars = this._createBarsAndLegends(data!, barHeight!, palette, barBackgroundColor, href);
    const showRatio = hideNumberDisplay === false && (!overrideFixStyle && data!.chartData!.length === 2);
    const showNumber = hideNumberDisplay === false && (!overrideFixStyle && data!.chartData!.length === 1);
    let total = 0;
    if (showRatio === true) {
      total = data!.chartData!.reduce((acc: number, value: IChartDataPoint) => acc + (value.data ? value.data : 0), 0);
    }
    const showLegend = hideLegend === false && (overrideFixStyle || data!.chartData!.length > 2);
