...
`FLUENT_CDN_BASE_URL` is generated from the shared metadata's `cdnBaseUrl` during builds through
the generator owned by `react-file-type-icons`. Until the metadata export is released, it reads
the copied JSON fixture described in that package's README. The metadata package is
