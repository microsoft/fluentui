...
export interface IBasePickerState<T extends IObjectWithKey> {
  items: T[];
...
function getStyledSuggestions<T>(suggestionsType: new (props: ISuggestionsProps<T>) => Suggestions<T>) {
  return styled<ISuggestionsProps<T>, ISuggestionsStyleProps<T>, ISuggestionsStyles>(
    suggestionsType,
    suggestionsStyles,
    undefined,
    {
 ...
 * {@docCategory Pickers}
 */
export class BasePicker<T extends IObjectWithKey, P extends IBasePickerProps<T>>
  ...
  constructor(basePickerProps: P) {
    ...
    // Typescript doesn't seem to properly deduce the typings here
    this.selection = new Selection<IObjectWithKey>({ onSelectionChanged: () => this.onSelectionChange() }) as Selection<
      T
    >;
  ...
  public UNSAFE_componentWillUpdate(newProps: P, newState: IBasePickerState<T>): void {
  ...
  public dismissSuggestions = (ev?: React.SyntheticEvent): void => {
  ...
  protected updateSuggestions(suggestions: T[]) {
    ...
    this.setState(
      ...
      () => {
        if (this.props.onGetMoreResults && this.input.current) {
          const suggestions: T[] | PromiseLike<T[]> = this.props.onGetMoreResults(
  ...
  protected addItem = (item: T): void => {
    const processedItem: T | PromiseLike<T> | null = this.props.onItemSelected ? this.props.onItemSelected(item) : item;
  ...
  protected removeItem = (item: T, focusNextItem?: boolean): void => {
    const { items } = this.state;
  ...
  protected removeItems = (itemsToRemove: T[]): void => {
    const { items } = this.state;
    const newItems: T[] = items.filter(item => itemsToRemove.indexOf(item) === -1);
