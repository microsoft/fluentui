...
export interface IGroupedListSectionProps<T = unknown> {
  ...
  eventsToRegister?: { eventName: string; callback: (context: IDragDropContext, event?: unknown) => void }[];
...
export class GroupedListSection<T = unknown> extends React.Component<
  IGroupedListSectionProps<T>,
...
> {
  ...
  private _onRenderGroupCell(
    onRenderCell: (depth: number | undefined, item: T, itemIndex: number | undefined) => React.ReactNode,
    groupNestingDepth: number | undefined,
  ): (item: T, itemIndex: number | undefined) => React.ReactNode {
    return (item: T, itemIndex: number | undefined): React.ReactNode => {
