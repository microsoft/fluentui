...
import { customizable, Customizations } from './index';
...
describe('Customizer wraps ThemeProvider', () => {
  it('provide theme using ThemeProvider', () => {
    safeMount(
      <Customizer settings={{ theme: { fonts: { medium: { fontWeight: 800 } } } }}>
        <div>Content</div>
      </Customizer>,
      wrapper => {
        expect(wrapper.html()).toEqual('<div><div>Content</div></div>');
        const inlineStyle = wrapper.find('div').get(0).props.style;
        expect(inlineStyle).toBeDefined();
        expect(Object.keys(inlineStyle).length).toBeGreaterThan(0);
      },
    );
  });
