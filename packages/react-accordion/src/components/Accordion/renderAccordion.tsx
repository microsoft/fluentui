...
export const renderAccordion = (state: AccordionState) => {
  ...
  const [accordionContextValue, descendantsContextValue] = useAccordionContextValue(state);

  return (
    <slots.root {...slotProps.root}>
      <AccordionContext.Provider value={accordionContextValue}>
        <DescendantsProvider value={descendantsContextValue}>{state.children}</DescendantsProvider>
