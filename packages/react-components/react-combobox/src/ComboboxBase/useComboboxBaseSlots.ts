...
export const useComboboxBaseSlots = (props: ComboboxBaseProps, state: ComboboxState | DropdownState) => {
  ...
  if ((state as ComboboxState).input) {
    (state as ComboboxState).input = {
      ...
      value,
      ...(state as ComboboxState).input,
      ref: useMergedRefs((state as ComboboxState).input.ref, triggerRef as React.RefObject<HTMLInputElement>),
    };

    triggerSlot = (state as ComboboxState).input;
  } else {
    (state as DropdownState).button = {
      ...
      type: 'button',
      ...(state as DropdownState).button,
      ref: useMergedRefs((state as DropdownState).button.ref, triggerRef as React.RefObject<HTMLButtonElement>),
    };

    triggerSlot = (state as DropdownState).button;
  }
