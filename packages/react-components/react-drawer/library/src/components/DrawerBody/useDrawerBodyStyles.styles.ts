 ...
 * type has narrowed from `SlotClassNames<DrawerBodySlots>` to `{ root: string }`, and the
 * value is no longer the `fui-DrawerBody` BEM static (D16.1).
 *
 * The value is a class TOKEN, not a selector: `/` is legal inside a class name but
 * terminates it in selector position, so `'.' + drawerBodyClassNames.root` is invalid CSS.
 * Use `fuiSelector(drawerBodyClassNames.root)` from `@fluentui/react-utilities` (D16.5).
 *
