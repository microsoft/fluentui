...
describe('DrawerHeaderNavigation', () => {
  isConformant({
    ...
    //
    // `component-has-static-classnames-object` asserts `drawerHeaderNavigationClassNames`
    // still holds `fui-DrawerHeaderNavigation` AND that it is rendered. Both are false by
    // design: DECISIONS.md D16.1 removed the BEM statics, D16.5 narrowed the export to
    // `{ root }` and re-pointed it at the group marker. It is replaced by
  ...
  it('renders a default state', () => {
    ...
    expect(result.container).toMatchInlineSnapshot(`
      <div>
        <nav
          class="group/fui-drawer-header-navigation"
        >
