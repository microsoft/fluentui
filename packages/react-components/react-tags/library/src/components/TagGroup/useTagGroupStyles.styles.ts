...
export const useTagGroupStyles_unstable = (state: TagGroupState): TagGroupState => {
  ...
  // or any other — can style an element from this TagGroup's state, because `styles.root`
  // is hashed and unaddressable from outside this file. Every Tag / InteractionTag rendered
  // inside can now read the group's own `data-size` through
  ...
  state.root.className = clsx(tagGroupClassNames.root, 'group/fui-tag-group', styles.root, state.root.className);
