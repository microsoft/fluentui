...
export const useTagGroupStyles_unstable = (state: TagGroupState): TagGroupState => {
  ...
  // element from this TagGroup's state, because `styles.root` is hashed and unaddressable
  // from outside this file. Every Tag / InteractionTag rendered inside can now read the
  // group's own `data-size` through `@variant group-size-small/fui-tag-group { … }`
  ...
  state.root.className = clsx('group/fui-tag-group', tagGroupClassNames.root, styles.root, state.root.className);
