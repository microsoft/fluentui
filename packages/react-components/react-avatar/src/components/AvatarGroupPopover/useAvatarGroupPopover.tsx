...
import { slot, useControllableState } from '@fluentui/react-utilities';
...
export const useAvatarGroupPopover_unstable = (props: AvatarGroupPopoverProps): AvatarGroupPopoverState => {
  ...
  return {
    ...
    root: slot(
      ...
      { required: true, elementType: Popover },
    ),
    triggerButton: slot(props.triggerButton, {
      required: true,
      defaultProps: {
    ...
    popoverSurface: slot(props.popoverSurface, {
      required: true,
      defaultProps: {
        'aria-label': 'Overflow',
        tabIndex: 0,
