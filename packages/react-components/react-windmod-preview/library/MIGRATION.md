...
84 component subpaths are listed below, all kebab-case:
...
#### 29. `InfoButton` has no `inline` prop, and its popover is always in the top layer
...
#### 30. `InfoButton` moves focus to the popover, and closes when focus leaves to nothing
...
#### 31. `<InfoButton children={null}>` renders the default glyph
