...
#### 63. `Checkbox`, `Radio`, and `Switch` align their indicators to the first label line

The inline control layouts use a two-row grid, `1lh minmax(0, auto)`. The label spans both rows,
while the indicator centers in the first row. A wrapping label therefore keeps the indicator
aligned with its first line. The former label-offset calculations are removed; the grid expresses
the relationship directly. `Switch` with `labelPosition="above"` uses a separate stacked grid:
the label occupies the first row and the indicator the second.

The first-row height comes from the line-height, so the accepted 1/64px rendering residue described
in entry 62 can also reach the control's geometry. The `switch` scene has a ratified ceiling of
3,132 differing pixels and a one-pixel height allowance. These bounds reflect the grid layout,
replacing the earlier flex-centering measurements.
...
scene. Sixty-six of the ninety-three scenes hold that unconditionally. The remaining twenty-seven carry a
...
| `menu`, `menu-rtl`                                    | 443 / 432 (was 413 / 412) | I + X    | The same top-layer compositing class as the TagPicker rows; GPU-mode ceilings covered measured bimodal modes (`menu` 403–413, `menu-rtl` 402–412) **plus 20 px each of `layoutunit-1/64` (Decision X, entry 62)**. The menu ceiling includes a further 10 px for a third observed GPU raster mode in the same surface-edge class. Original shadow-quantisation mechanism unchanged and still primary.                                                                                                                                                                  | Both rows vary under the flag and carry no no-GPU expectation. **GPU gate authoritative**                                                                              |
...
| `switch`                       | 3132        | X        | Row-edge and label/track alignment from the first-line grid layout (entry 63), including a one-pixel height allowance.                                                                                                                                                                                                                                                                                                                                                     |
| `tag`                          | 2111        | X        | Row-edge.                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
