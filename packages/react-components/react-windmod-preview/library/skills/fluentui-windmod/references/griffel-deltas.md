...
The package's `MIGRATION.md` enumerates **sixty-three** deliberate differences in full. This is the
...
| 59  | **Leading is arithmetic, not a token** — `leading-<n>` is `n/100`, `leading-<a>/<b>` an exact ratio | Nothing to read back: no `--leading-*` custom property exists. Line height is a unitless ratio on the element's own font-size; descendants inherit the ratio, not a px box.                                                                                                                                                                                                |
| 60  | `layout="offset"` keeps the previous button first in the DOM                                        | The offset look comes from CSS `order`, not DOM order — the headless footer always renders previous, children, next. Tab and a screen reader reach the previous button before the page count, not after as in Griffel. Update assertions on tab/reading order.                                                                                                             |
| 63  | `Switch` centres its indicator against a wrapping label's whole block, not its first line           | Root moved from `items-start` to `items-center` for inline label positions (a no-op for single-line labels, since matching heights already rendered them identically). A wrapping label's indicator now centers on the full wrapped block instead of its first line, closer to `Checkbox`/`Radio`. Update snapshots of the old top-aligned geometry for multi-line labels. |
