...
export const useBadgeStyles_unstable = (state: BadgeState): BadgeState => {
  ...
  // element from this Badge's state, because `styles.root` is hashed and unaddressable from
  // outside this file. Badge needs no state mirrors: `data-size`, `data-icon-position` and
  // `data-empty` are already stamped on this very element above, so
  // `@variant group-size-tiny/fui-badge` etc. work as-is (DECISIONS.md D15, Tier 0).
  //
