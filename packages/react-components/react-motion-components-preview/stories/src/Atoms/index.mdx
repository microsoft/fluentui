...
<div className="warning-banner">
  <strong>⚠️ Advanced API:</strong> Motion atoms are intended for developers creating custom motion components. For most
  use cases, use the pre-built components like <code>Fade</code>, <code>Scale</code>, etc.
</div>

## What are Motion Atoms?

Motion atoms are **factory functions** that generate `AtomMotion` objects—the fundamental units of animation in the Fluent Motion System. Each atom produces a single animation effect (opacity, scale, translation, etc.) that can be used directly or composed together.
...
<table className="atom-table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        <code>outOpacity</code>, <code>inOpacity</code>
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        <code>outScale</code>, <code>inScale</code>
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        <code>outX</code>, <code>outY</code>, <code>inX</code>, <code>inY</code>
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        <code>axis</code>, <code>outAngle</code>, <code>inAngle</code>
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
        <code>outRadius</code>, <code>inRadius</code>
      </td>
    </tr>
  </tbody>
</table>
...
fadeAtom({
  direction: 'enter',
  duration: 200,
  outOpacity: 0, // Opacity for the out state (default: 0)
  inOpacity: 1, // Opacity for the in state (default: 1)
  easing: 'ease-out',
  delay: 0,
});
```
...
scaleAtom({
  direction: 'enter',
  duration: 200,
  outScale: 0.9, // Scale for the out state (default: 0.9)
  inScale: 1, // Scale for the in state (default: 1)
  easing: 'ease-out',
});
```
...
slideAtom({
  direction: 'enter',
  duration: 200,
  outX: '0px', // X translate for the out state (default: '0px')
  outY: '20px', // Y translate for the out state (default: '0px')
  inX: '0px', // X translate for the in state (default: '0px')
  inY: '0px', // Y translate for the in state (default: '0px')
  easing: 'ease-out',
});
```
...
rotateAtom({
  ...
  outAngle: -90, // Rotation angle for the out state in degrees (default: -90)
  inAngle: 0, // Rotation angle for the in state in degrees (default: 0)
  easing: 'ease-out',
});
```
...
blurAtom({
  direction: 'enter',
  duration: 200,
  outRadius: '10px', // Blur radius for the out state (default: '10px')
  inRadius: '0px', // Blur radius for the in state (default: '0px')
  easing: 'ease-out',
});
```
...
const FadeSlide = createPresenceComponent({
  enter: [
    ...
    slideAtom({ direction: 'enter', duration: 200, outY: '20px' }),
  ],
  exit: [fadeAtom({ direction: 'exit', duration: 150 }), slideAtom({ direction: 'exit', duration: 150, outY: '20px' })],
});
```
...
const DynamicSlide = createPresenceComponent(({ element }) => {
  ...
  return {
    enter: slideAtom({
      direction: 'enter',
      duration: 200,
      outX: `-${width}px`, // Slide by element's own width
    }),
    exit: slideAtom({
      direction: 'exit',
      duration: 200,
...
<div className="info-box">
  <strong>💡 Tip:</strong> When composing atoms, ensure all atoms in an array use the same duration for a cohesive
