...
**Slide** supports four directions for enter/exit:

- `from="left"` — Slides in from left, out to left
- `from="right"` — Slides in from right, out to right
- `from="top"` — Slides in from top, out to top
- `from="bottom"` — Slides in from bottom, out to bottom
...
<Fade visible={isVisible} fromOpacity={0.5} toOpacity={1}>
  <Content />
</Fade>

// Custom scale values
<Scale visible={isVisible} fromScale={0.8} toScale={1}>
  <Content />
</Scale>

// Custom slide direction and distance
<Slide visible={isVisible} from="right" distance="100px">
  <Content />
</Slide>
...
| ---------- | ------------------------------------------------------ | -------------------------------------------------- |
| `Fade`     | `FadeSnappy`, `FadeRelaxed`                            | `fromOpacity`, `toOpacity`, `duration`, `easing`   |
| `Scale`    | `ScaleSnappy`, `ScaleRelaxed`                          | `fromScale`, `toScale`, `duration`, `easing`       |
| `Collapse` | `CollapseSnappy`, `CollapseRelaxed`, `CollapseDelayed` | `orientation`, `duration`, `easing`                |
| `Slide`    | `SlideSnappy`, `SlideRelaxed`                          | `from`, `distance`, `duration`, `easing`           |
| `Blur`     | —                                                      | `radius`, `duration`, `easing`                     |
| `Rotate`   | —                                                      | `axis`, `angle`, `duration`, `easing`              |
