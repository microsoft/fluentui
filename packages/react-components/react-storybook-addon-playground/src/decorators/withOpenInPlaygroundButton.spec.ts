...
describe('withOpenInPlaygroundButton', () => {
  ...
  it('opens the playground with the story source encoded in the hash', () => {
    ...
    expect(decodePlaygroundStateFromHash(`#${hash}`)).toEqual({ code: fullSource, cssModules: [] });
  });
  ...
  it('encodes story CSS modules in the playground URL', () => {
    ...
    expect(decodePlaygroundStateFromHash(`#${url.split('#')[1]}`)).toEqual({ code: fullSource, cssModules });
  });
