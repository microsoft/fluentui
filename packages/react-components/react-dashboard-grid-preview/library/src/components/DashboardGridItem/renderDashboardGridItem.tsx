...
export const renderDashboardGridItem_unstable = (
  state: DashboardGridItemInternalState,
): JSXElement => {
  ...
  return (
    <DashboardGridItemContextProvider value={state.contextValue}>
      <state.root>
        ...
        {ResizeHandle &&
          state.resizeDirections.map(edge => (
            <ResizeHandle key={edge} {...state.resizeHandles[edge]} />
          ))}
      </state.root>
