...
export const cacheCurrentLayout = (
  state: EngineState,
  clear = false,
): void => {
  if (clear) {
...
export const cacheAuthoredLayout = (
  ...
  state.referenceColumns = Math.max(
    state.referenceColumns,
    normalizedColumns,
  );
  const target =
    state.layouts.get(normalizedColumns) ??
    new Map<OpaqueNodeKey, CachedLayoutNode>();
  target.set(key, { ...layout });
  state.layouts.set(normalizedColumns, target);
};

export const removeNodeFromAllLayouts = (
  state: EngineState,
  key: OpaqueNodeKey,
): void => {
...
export const synchronizeResponsiveCaches = (
  before: EngineState,
  after: EngineState,
): void => {
  ...
  after.layouts.forEach((layout, columns) => {
    ...
    after.nodes.forEach(node => {
      ...
      if (node.y !== previous.y) {
        cached.y = Math.max(
          0,
          (cached.y ?? previous.y) + node.y - previous.y,
        );
        delete cached.auto;
      }
  ...
  try {
    output = layout(
      Object.freeze({
        ...
        columns,
        items: Object.freeze(
          sortNodesStable(state.nodes).map(nodeToResolvedItem),
        ),
      }),
    );
  } catch (error) {
    diagnostics.push(
      createDiagnostic(
        ...
        'A custom column layout threw while computing the target layout.',
        {
          severity: 'error',
          details: {
...
const sourceForNode = (
  ...
  sourceLayout?.get(node.key) ?? { x: node.x, y: node.y, w: node.w };
  ...
  if (failed) {
    return {
      diagnostic: createDiagnostic(
        'collision-cycle',
        'Responsive layout collision repair exceeded its pass budget.',
        { severity: 'error' },
      ),
    };
  }
...
export const applyColumnChange = (
  ...
  const exactTarget =
    columns > previousColumns ? state.layouts.get(columns) : undefined;
  ...
  const baseColumns = exactTarget !== undefined
    ? columns
    : useLargest
      ? largestColumns
      : previousColumns;
  const sourceLayout = exactTarget ?? (useLargest
    ? state.layouts.get(largestColumns)
    : layoutFromNodes(state.nodes));
  ...
  if (columns === 1) {
    ...
    candidates = state.nodes.map(source => {
      ...
      if (exactTarget !== undefined) {
        ...
        node.x = move
          ? Math.round((cached.x ?? node.x) * ratio)
          : Math.min(cached.x ?? node.x, columns - 1);
        node.w =
          baseColumns === 1
            ? 1
            : scale
              ? Math.max(1, Math.round(cached.w * ratio))
              : Math.min(cached.w, columns);
        node.y = cached.y ?? node.y;
        if (cached.auto) {
          node.auto = true;
        }
    ...
    if (rebuilt.nodes === undefined) {
      return {
        ...
        diagnostics: Object.freeze(
          rebuilt.diagnostic === undefined ? [] : [rebuilt.diagnostic],
        ),
      };
    }
  ...
  if (
    ...
    return {
      ...
      diagnostics: Object.freeze([
        createDiagnostic(
          'max-rows',
          'The responsive layout exceeds the configured maximum row count.',
          { severity: 'error' },
        ),
      ]),
    };
...
export const getCachedLayout = (
  state: EngineState,
  columns: number,
): ReadonlyMap<OpaqueNodeKey, CachedLayoutNode> | undefined =>
...
export const setCachedLayout = (
  ...
  state.layouts.set(
    columns,
    new Map(
      [...layout].map(([key, node]) => [key, { ...node }] as const),
    ),
  );
  state.referenceColumns = Math.max(state.referenceColumns, columns);
};
