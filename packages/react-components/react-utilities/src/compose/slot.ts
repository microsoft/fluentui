...
export function slot<Props extends UnknownSlotProps>(
  ...
  const metadata: SlotComponentMetadata<Props> = isSlot<Props>(value)
    ? value[SLOT_COMPONENT_METADATA_SYMBOL]
    : {
        elementType: elementType ?? ('div' as React.ElementType<Props> as React.ComponentType<Props>),
      };
...
/**
 * Guard method to ensure a given element is a slot. This is mainly used internally
 */
...
export function getSlotMetadata<P extends UnknownSlotProps>(
  value: P | SlotComponent<P> | SlotShorthandValue | undefined | null,
): SlotComponentMetadata<P> {
  if (isSlot<P>(value)) {
    return value[SLOT_COMPONENT_METADATA_SYMBOL];
  }
  return { elementType: 'div' as React.ElementType<P> as React.ComponentType<P> };
}
