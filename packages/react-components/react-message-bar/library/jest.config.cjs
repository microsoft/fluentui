...
module.exports = {
   ...
   *
   * Griffel's jest serializer was kept alongside it (retired in the S-J closing batch — icons 3.0 emits no Griffel classes): MessageBar's own suites render
   * `@fluentui/react-motion-components-preview` presence components and the stories/e2e
   * render Link and Button content, and unconverted packages elsewhere in the tree still
   * emit Griffel atomics into the same `class=` attribute.
   */
  moduleNameMapper: {
