...
export const MessageBarBody: ForwardRefComponent<MessageBarBodyProps> = React.forwardRef((props, ref) => {
  let state = useMessageBarBody_unstable(props, ref);
  const ctx = useMessageBarBodyContextValues_unstable(state);

  state = useMessageBarBodyStyles_unstable(state);
  state = useCustomStyleHook_unstable('useMessageBarBodyStyles_unstable')(state);
