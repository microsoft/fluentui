 ...
 * a converted styles file calls no React hook and no RSC-unsafe function (`makeStyles` and
 * `makeResetStyles` are gone), so `enforce-use-client` is right that `'use client'` is now
 * unnecessary. It is kept because migration/griffel-to-tailwind/CONVERSION_GUIDE.md §3 makes
 * a conversion a pure styling change; dropping directives is a Phase 3 sweep across all 180
 * style hooks.
 *
 * The suppression is a trailing `eslint-disable-line` rather than a leading `eslint-disable`
 * block because a leading block comment pushes `'use client'` off the first line of the
 * emitted lib/lib-commonjs output — every other v9 source file in the repo has the directive
 * at line 1.
 */
