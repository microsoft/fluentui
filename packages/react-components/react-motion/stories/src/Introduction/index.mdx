...
<div className="principle-card">
  ...
  <p>
    ...
    engine. No runtime physics calculations—just declarative keyframes.
  </p>
</div>
...
- **Accelerate curves** — Elements leaving the screen (exit motions)
- **Decelerate curves** — Elements entering the screen (enter motions)
- **Easy-ease curves** — Elements moving within the screen
...
<Slide visible={showDrawer} direction="right">
  <Drawer />
</Slide>
```
...
├── createMotionComponent()      — Factory for one-way motion
├── createPresenceComponent()    — Factory for two-way presence
├── PresenceGroup                — Animate lists of items
├── motionTokens                 — Duration and curve values
└── presenceMotionSlot           — Integration with Fluent components
...
├── Blur
├── Rotate
└── Stagger                      — Choreography for sequential animations
```
