...
export type MotionComponentProps<MotionKind extends string> = {
 ...
 *
 * @param motionByKind - A motion definition.
 */
export function createMotionKindComponent<
  MotionKind extends string,
  MotionParams extends Record<string, MotionParam> = {},
>(motionByKind: Record<MotionKind, AtomMotion | AtomMotion[] | AtomMotionFn<MotionParams>>) {
  const Atom: React.FC<MotionComponentProps<MotionKind> & MotionParams> = props => {
    'use no memo';
    ...
    const params = _rest as Exclude<typeof props, MotionComponentProps<MotionKind>>;
    ...
    useIsomorphicLayoutEffect(() => {
      ...
      if (element) {
        const value = motionByKind[motionKind];
        const atoms =
          typeof value === 'function'
            ? value({ element, ...optionsRef.current.params })
            : (value as AtomMotion | AtomMotion[]);
    ...
    }, [animateAtoms, handleRef, motionKind, isReducedMotion, onMotionFinish, onMotionStart, onMotionCancel]);
