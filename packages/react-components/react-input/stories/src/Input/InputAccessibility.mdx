...
1. For application-level search inputs that have a search icon -- the search icon is considered generally understood and can function as the visible label for one search input in the page or app. If there are multiple search inputs with different functions, they will still require a specific text label, however.
