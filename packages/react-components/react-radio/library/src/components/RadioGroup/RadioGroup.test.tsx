...
describe('RadioGroup', () => {
  isConformant({
    ...
    //
    // `component-has-static-classnames-object` asserts that `radioGroupClassNames` holds
    // `fui-RadioGroup` and that the class is rendered. RadioGroup publishes neither any more:
    // the BEM static is removed and `radioGroupClassNames` re-points to the named group
    // marker this component now stamps (DECISIONS.md D15.1/D16.1/D16.5). The test is disabled
    // per package as each one is swept; it leaves `defaultTests` altogether once every
