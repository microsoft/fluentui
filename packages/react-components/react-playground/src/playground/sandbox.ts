...
(() => {
  ...
  window[callbackName] = nextRuntime => {
    ...
    send({
      ...
      metadata: {
        ...
        themes: (setup.themes || []).map(theme => ({ id: theme.id, label: theme.label })),
      },
    });
  };
  ...
  window.addEventListener('message', async event => {
    ...
    try {
      ...
      const unavailable = requested.filter(name => !runtime.moduleLoaders[name]);
      ...
      const loaded = await Promise.all(
        requested.map(async name => [name, await runtime.moduleLoaders[name]()])
      );
