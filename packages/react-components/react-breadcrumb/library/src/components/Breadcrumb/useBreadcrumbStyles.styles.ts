...
export const useBreadcrumbStyles_unstable = (state: BreadcrumbState): BreadcrumbState => {
  ...
  // from this Breadcrumb's state, because a `*.module.css` class is hashed and unaddressable
  // from outside its own file. Read it as `@variant group-…/fui-breadcrumb { … }`
  ...
  state.root.className = clsx('group/fui-breadcrumb', breadcrumbClassNames.root, state.root.className);
