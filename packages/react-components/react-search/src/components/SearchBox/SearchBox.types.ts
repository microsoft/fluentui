...
export type SearchBoxSlots = {
   ...
   *
   * The root slot receives the `className` and `style` specified directly on the `<SearchBox>`.
   ...
   *
   * This is the "primary" slot, so native props specified directly on the `<SearchBox>` will go here
  ...
  /** Last element within the input border */
  dismiss?: Slot<'span'>;
};
...
> & {
   ...
   */
  onChange?: (ev: React.ChangeEvent<HTMLInputElement>, data: SearchBoxOnChangeData) => void;

  /**
   * An input can have different text-based [types](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/SearchBox#input_types)
 ...
 */
export type SearchBoxOnChangeData = {
