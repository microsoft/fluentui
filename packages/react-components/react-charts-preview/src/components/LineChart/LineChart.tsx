...
//import { ILine } from '@fluentui/react';
...
export const LineChart: React.FunctionComponent<ILineChartProps> = React.forwardRef<HTMLDivElement, ILineChartProps>(
  (props, forwardedRef) => {
    ...
    const [nearestCircleToHighlight, setNearestCircleToHighlight] = React.useState<null>(null);
    const [activeLine, setActiveLine] = React.useState<null>(null);
    ...
    function _createLegends(data: LineChartDataWithIndex[]): JSX.Element {
      ...
      const _initializeLineChartData = (
        xScale: NumericAxis,
        yScale: NumericAxis,
    ...
    if (isSelectedLegend) {
      ...
      for (let i = 0; i < _colorFillBars.length; i++) {
        ...
        for (let j = 0; j < colorFillBar.data.length; j++) {
          ...
          const opacity =
            _legendHighlighted(colorFillBar.legend) || _noLegendHighlighted() || state.isSelectedLegend
    ...
    const pointToHighlightUpdated =
      ...
      (nearestCircleToHighlight !== null &&
        ...
        (nearestCircleToHighlight.x !== pointToHighlight.x ||
          nearestCircleToHighlight.y !== pointToHighlight.y));
    // if no points need to be called out then don't show vertical line and callout card
    if (found && pointToHighlightUpdated) {
      ...
      d3Select(`#${_verticalLine}`)
        .attr(
          'transform',
          () => `translate(${_xAxisScale(pointToHighlight.x)}, ${_yAxisScale(pointToHighlight.y)})`,
        )
        .attr('visibility', 'visibility')
    ...
    if (!found) {
      ...
      function _handleMultipleLineLegendSelectionAction(selectedLine: LineChartDataWithIndex) {
        ...
        if (
          ...
          ((props.colorFillBars && props.colorFillBars.length === selectedColorBarLegend.length) ||
            !props.colorFillBars)
        ) {
      ...
      const { tickValues, tickFormat, eventAnnotationProps } = props;
      _points = _injectIndexPropertyInLineChartData(props.data.lineChartData);
