...
describe('v8 TimePicker tests', () => {
  ...
  it('calls onValidationResult only when the error message changes', () => {
    ...
    const onValidationResult = jest.fn((_ev, timePickerErrorData): void => {
  ...
  describe('validates entered text when', () => {
    it('receives an invalid hour input for 24-hour-no-seconds format', () => {
      ...
      expect(container.querySelector('#test-error')?.textContent).toMatch('24-hour format: hh:mm');
    });

    it('receives an invalid hour input for 24-hour-with-seconds format', () => {
      const { getByRole, container } = render(
        <TimePicker id="test" allowFreeform useHour12={false} showSeconds={true} />,
      );

      const input = getByRole('combobox');
      userEvent.type(input, '24:22:42{enter}');

      expect(container.querySelector('#test-error')?.textContent).toMatch('24-hour format: hh:mm:ss');
    });

    it('receives an invalid hour input for 12-hour-no-seconds format', () => {
      const { getByRole, container } = render(
        <TimePicker id="test" allowFreeform useHour12={true} showSeconds={false} />,
      );

      const input = getByRole('combobox');
      userEvent.type(input, '13:26 PM{enter}');

      expect(container.querySelector('#test-error')?.textContent).toMatch('12-hour format: hh:mm AP');
    });
    ...
    it('receives an invalid hour input for 12-hour-with-seconds format', () => {
      const { getByRole, container } = render(
        <TimePicker id="test" allowFreeform useHour12={true} showSeconds={true} />,
      );

      const input = getByRole('combobox');
      userEvent.type(input, '15:26:37 AM{enter}');

      expect(container.querySelector('#test-error')?.textContent).toMatch('12-hour format: hh:mm:ss AP');
    });
