...
2. an option object that would alow you to control open and checked state
   * \`openItems\` and \`defaultOpenItems\` (controlling open state)
   * \`checkedItems\` and \`defaultCheckedItems\` (controlling checked state)
   * \`rootValue\` and \`rootLevel\` (properties to indicate that the hook root item is not the root of the tree itself, useful in the cases where you want to split the hook into multiple utilizations)
`,
    },
  },
};
