...
export const useRatingStyles_unstable = (state: RatingState): RatingState => {
  ...
  // or any other — can style an element from this Rating's state, because `styles.root` is
  ...
  state.root.className = clsx(ratingClassNames.root, 'group/fui-rating', styles.root, state.root.className);
