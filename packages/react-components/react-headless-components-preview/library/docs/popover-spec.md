...
// Surface (top layer) — <dialog> by default; may be <div> when trapFocus is false
<SurfaceElement
  id={surfaceId}
...
`PopoverSurface` renders as a native **`<dialog>`** by default. Consumers can set `as="div"` for a non-modal surface. Both supported non-modal roots receive `popover="auto"` and open with `showPopover()`.

When `trapFocus={true}`, the surface must remain a `<dialog>` and opens with `showModal()`. Combining `as="div"` with `trapFocus` is unsupported and produces a development warning because a `<div>` does not implement the modal dialog API.

The default element follows the [Open UI popover-vs-dialog research](https://open-ui.org/components/popover.research.explainer/#popover-vs-dialog), which recommends `<dialog popover>` for top-layer non-modal cases — it preserves popover-style stacking and light-dismiss in the default path while leaving modal trap as a one-method swap when consumers opt in.
...
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `false`     | Explicit `role="group"`, overriding `<dialog>`'s implicit `role="dialog"` when the default root is used.                                 | Non-modal anchored container suitable for menus, cards, and informational overlays. Matches the prior contract.                 |
| `true`      | Explicit `role="dialog"` by default (or another valid role if a consumer overrides it via props); `showModal()` supplies modal behavior. | Modal dialog semantics for assistive tech; the surface must use the default `<dialog>` root so the browser can open it modally. |
...
`openOnContext` cannot call `setOpen(true)` synchronously from the `contextmenu` handler. The right-click sequence (`pointerdown` → `contextmenu` → `pointerup` → `auxclick`, plus on some platforms a synthetic `click`) is not finished when `contextmenu` fires. Because the surface is rendered with `popover="auto"` and is opened **imperatively** via `showPopover()` — i.e. without an invoker association (no `popovertarget` attribute on the trigger) — trailing pointer/click events that arrive after opening can be treated by the browser's light-dismiss algorithm as outside-clicks. The browser then fires `toggle({ newState: 'closed' })` and the surface snaps shut immediately after opening.

To avoid this, `PopoverTrigger`'s `onContextMenu` handler:

1. Calls `e.preventDefault()` to suppress the native menu.
2. Registers a one-shot capture-phase `pointerup` listener on `targetDocument`.
3. When that `pointerup` listener fires, it calls `setOpen(nativeEvent, true)` synchronously so the open is committed after the `contextmenu` event itself, rather than during it.

This implementation defers opening until `pointerup`, which avoids opening during the `contextmenu` event. A future iteration that adopts `popovertarget` invoker association on the trigger could let us simplify this path further, since the browser would then recognize the trigger's events as belonging to the popover.
