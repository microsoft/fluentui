...
const fixture = process.argv[2] ?? 'OverlayRuntime.fixture.js';
const outputPath = path.join(
  packageRoot,
  'dist',
  'overlay-runtime-bundle',
  path.parse(fixture).name,
);
const entry = path.join(packageRoot, 'bundle-size', fixture);

const isFallbackRuntime = resource =>
  resource.includes(`${path.sep}overlayRuntime${path.sep}fallbackRuntime.`);
...
class VerifyOverlayRuntimeChunksPlugin {
  apply(compiler) {
    compiler.hooks.afterEmit.tap('VerifyOverlayRuntimeChunksPlugin', compilation => {
      ...
      if (fallbackChunks.size !== 1) {
        compilation.errors.push(
          new Error(`Expected one async overlay fallback boundary, found ${fallbackChunks.size}.`),
        );
      } else if ([...fallbackChunks][0].canBeInitial()) {
        compilation.errors.push(new Error('Overlay fallback runtime was retained in an initial chunk.'));
      }
...
const compiler = webpack({
  ...
  plugins: [new VerifyOverlayRuntimeChunksPlugin()],
});
...
compiler.run((error, stats) => {
  compiler.close(() => {
    ...
    console.log(`${fixture}: overlay fallback runtime is isolated in one asynchronous boundary.`);
  });
});
