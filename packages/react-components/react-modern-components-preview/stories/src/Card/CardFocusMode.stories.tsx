...
import { makeStyles, Button, Caption1, Body1, Subtitle1, useFocusableGroup } from '@fluentui/react-components';
import { MoreHorizontal20Regular, Open16Regular, Share16Regular } from '@fluentui/react-icons';
import { Card, CardHeader, CardFooter, CardPreview } from '@fluentui/react-modern-components-preview/card';
...
type FocusMode = 'off' | 'no-tab' | 'tab-exit' | 'tab-only';
...
const CardExample = ({ focusMode = 'off', ...props }: CardProps & { focusMode?: FocusMode }) => {
  const styles = useStyles();
  const focusAttributes = useFocusableGroup({ tabBehavior: focusMap[focusMode] });

  return (
    <Card
      className={styles.card}
      {...(focusMode === 'off' ? undefined : focusAttributes)}
      tabIndex={focusMode === 'off' ? undefined : 0}
      {...props}
    >
      <CardPreview>
...
export const FocusMode = (): React.ReactNode => {
  const styles = useStyles();
