...
Advanced usage includes things like passing complex custom styles objects, callbacks for custom rendering, using refs to make imperatives calls, and complex data binding.
...
_Incremental_: upgrade a few components and ship, often with flighting the upgraded components to a subset of users.
...
_Shallow_: upgrading components that are appear in only one non-critical place in your application.
...
_Shims_: author or use a component that take v8 props and renders a v9 component.
...
Plan a tasks or set of tasks to be able to 'rinse and repeat' each usage upgrade.
