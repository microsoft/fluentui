...
If you need to completely change the layout, appearance, and behavior of a component part,
custom styling and theming might not be sufficient.
...
A property of a component can be identified as a slot by its type.

- IntrinsicShorthandProps: Defines the props for a slot that supports one or more intrinsic element types, such as 'div'.
- ObjectShorthandProps: Defines the props for a slot that supports a Component type.
- ShorthandRenderFunction: Defines a render function for a slot.

Notice these types contain the term 'shorthand'.
Shorthand means the slot props can accept 'shorthand' data such as primitive values and props objects.
