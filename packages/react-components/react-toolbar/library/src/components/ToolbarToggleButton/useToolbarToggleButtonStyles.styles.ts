...
export const useToolbarToggleButtonStyles_unstable = (state: ToolbarToggleButtonState): ToolbarToggleButtonState => {
  ...
  //
  // Unlike the converted leaf components there is no static class to sit beside, and no
  // unconditional module class either: this root IS react-button's ToggleButton root, and
  // `useToggleButtonStyles_unstable` (called below) contributes `fui-ToggleButton` +
  // `group/fui-toggle-button`, then `useButtonStyles_unstable` adds `fui-Button` +
  // `group/fui-button`, all to the same element. Three markers on one element is correct —
  // it genuinely is all three things, and a descendant can address whichever identity it
  // means.
  //
  // Ordering here is per-argument within THIS clsx; because the ToggleButton hook runs
  // afterwards and PREPENDS its own arguments, `fui-ToggleButton` is the rendered
  // `classList[0]` and the D15.1 invariant (the marker is never the first class token —
  // nwsapi's `:scope` polyfill throws on the `/` under jsdom) holds on the string that
  // actually reaches the DOM. Same reasoning as ToolbarButton's hook.
  //
