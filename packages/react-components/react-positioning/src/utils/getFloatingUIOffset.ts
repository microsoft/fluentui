...
import type { Rect, Placement } from '@floating-ui/dom';
 ...
 * Type taken from Floating UI since they are not exported
 */
export type FloatingUIOffsetFunction = (args: {
  floating: Rect;
  reference: Rect;
  placement: Placement;
...
export function getFloatingUIOffset(
  ...
  return ({ floating, reference, placement }) => {
    const { position, alignment } = fromFloatingUIPlacement(placement);
    return rawOffset({ positioned: floating, target: reference, position, alignment });
  };
}
