...
import { SharedColors, NeutralColors } from '../IbizaColors';
...
export const DropdownStyles = (props: IDropdownStyleProps): Partial<IDropdownStyles> => {
  ...
  const titleOpenBorderRadius =
    calloutRenderEdge === RectangleEdge.bottom ? `${borderRadius} ${borderRadius} 0 0` : `0 0 ${borderRadius} ${borderRadius}`;

  const calloutOpenBorderRadius =
    calloutRenderEdge === RectangleEdge.bottom ? `0 0 ${borderRadius} ${borderRadius}` : `${borderRadius} ${borderRadius} 0 0`;
    ...
    title: [
      {
        ...
        borderRadius: isOpen ? titleOpenBorderRadius : borderRadius,
