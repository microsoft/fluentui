...
describe('compileSwc dynamic imports', () => {
  beforeEach(() => {
    transformFileMock.mockClear();
    (readJsonFile as jest.Mock).mockClear();
    (globSync as jest.Mock).mockClear();
  });

  it('preserves dynamic imports for opted-in CommonJS output', async () => {
    await compileSwc({ module: 'commonjs', outputPath: 'lib-commonjs' }, createOptions(true));
  ...
  it('keeps existing CommonJS behavior by default', async () => {
    await compileSwc({ module: 'commonjs', outputPath: 'lib-commonjs' }, createOptions(false));
