...
- [ ] 4.3 Apply per-page wrappers for the v9 pages that use Storybook `decorators`. The pages render correctly without them, so this is a layout-fidelity gap rather than a break: e.g. Field's decorator constrains examples to 400px and Slider's applies a grid. `<ComponentPage wrapper>` already exists; each page needs the prop set.
