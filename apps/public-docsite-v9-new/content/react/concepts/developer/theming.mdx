...
You can browse all the available tokens in **[Theme](/react/theme/colors)** section of the docs.
...
Place a `<FluentProvider />` at the root of your app and pass a theme to the `theme` prop. The provider will render a `div` and set all tokens as CSS variables on that element. The provider also propagates CSS variables to React portals created with [Portal component](/react/components/portal).
...
For more details, see [Styling components](/react/concepts/developer/styling-components).
