...
Like `Button`, `MenuButton` requires an accessible label. The label rules from [Button](/react/concepts/developer/accessibility/components/button) apply unchanged.
...
When an icon-only `MenuButton` has both an `icon` slot and no children, the `menuIcon` is omitted from the rendered output (see [renderMenuButton.tsx](https://github.com/microsoft/fluentui/blob/master/packages/react-components/react-button/library/src/components/MenuButton/renderMenuButton.tsx#L20)) so the button does not render two icons.
