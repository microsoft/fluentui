...
Like `Button`, `MenuButton` requires an accessible label. The label rules from [Button](?path=/react/concepts/developer/accessibility/components/button) apply unchanged.
