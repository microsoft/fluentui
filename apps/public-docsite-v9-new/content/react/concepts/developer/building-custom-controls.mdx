...
For detailed accessibility guidance, see [Accessibility](?path=/docs/react/concepts/developer/accessibility/components-overview).

### Relationship to other customization options

- Use [Styling Components](?path=/docs/react/concepts/developer/styling-components) for standard style overrides.
- Use [Advanced Styling Techniques](?path=/docs/react/concepts/developer/advanced-styling-techniques) for app-wide style hook customization.
- Use [Customizing Components with Slots](?path=/docs/react/concepts/developer/customizing-components-with-slots) for part-level composition within existing components.
