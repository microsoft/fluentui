...
export async function packProjectPackages(logger: Function): Promise<PackedPackages> {
  ...
  const lernaProject = new Project(config.paths.allPackages());
  const projectPackages: any = await lernaProject.getPackages();
  ...
  const requiredPackages = flattenPackageGraph(['@fluentui/react'], projectPackagesGraph);
