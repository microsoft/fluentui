...
import { usePubSub, PubSubProvider as _PubSubProvider } from '@cactuslab/usepubsub';
import type { Handler } from '@cactuslab/usepubsub';
// '@cactuslab/usepubsub' doesn't ship React 18 compatible types
const PubSubProvider = _PubSubProvider as React.ComponentType<
...
const ValidationMessage = ({ id, formValidation, children }: ValidationMessageProps): React.ReactElement => {
  const [isAlerting, setIsAlerting] = React.useState(true);

  const alert = React.useCallback(() => {
    setIsAlerting(false);
    globalThis.setTimeout(() => setIsAlerting(true), 200);
  }, [setIsAlerting]);
...
const FormExample = ({ variant }: FormExampleProps): React.ReactElement => {
  ...
  React.useEffect(() => {
    ...
    const firstErrorField = targetDocument?.getElementById(`${variant}-${firstErrorName}`);
    if (firstErrorField) {
      targetDocument?.defaultView?.setTimeout(() => firstErrorField.focus(), 500);
    }
  }, [variant, errors, formState, formValidation, targetDocument]);

  React.useEffect(() => {
    if (isSubmittedAndValid) {
      targetDocument?.getElementById(`${variant}-validMessage`)?.focus();
    }
  }, [variant, isSubmittedAndValid, targetDocument]);
...
export const FormErrorLabelBad = (): React.ReactElement => (
  <PubSubProvider>
    <FormExample variant="bad" />
  </PubSubProvider>
);

export const FormErrorLabelGood = (): React.ReactElement => (
  <PubSubProvider>
