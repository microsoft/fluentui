...
For sublabels or extra descriptive text per radio, render the additional text outside the `label` slot and wire it to the radio via `aria-describedby`. The [RadioGroupLabelSubtext](/react/components/radio-group#label-subtext) story demonstrates this pattern.
