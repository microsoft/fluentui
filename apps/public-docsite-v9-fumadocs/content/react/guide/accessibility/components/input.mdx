...
This is the consumer's responsibility — `Input` does not infer the accessible name from `contentBefore` / `contentAfter`. The [ContentBeforeAfter](/react/components/input#content-before-after) story demonstrates all three patterns.
...
When `Input` is inside `Field` with `validationState="error"`, `aria-invalid="true"` is applied to the `<input>` and the wrapper switches to the red border style. Outside of `Field`, the consumer can set `aria-invalid` directly — the red border style keys off the rendered `aria-invalid` attribute on the `<input>` ([useInputStyles.styles.ts:336](https://github.com/microsoft/fluentui/blob/master/packages/react-components/react-input/library/src/components/Input/useInputStyles.styles.ts#L336)), so styling and ARIA stay in sync.
