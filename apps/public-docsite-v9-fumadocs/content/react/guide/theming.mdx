...
import asset0 from '@repo/apps/public-docsite-v9/public/brand-ramp-example.png';
...
You can browse all the available tokens in **[Foundations](/react/foundations/colors)** section of the docs.
...
Place a `<FluentProvider />` at the root of your app and pass a theme to the `theme` prop. The provider will render a `div` and set all tokens as CSS variables on that element. The provider also propagates CSS variables to React portals created with [Portal component](/react/components/portal/portal).
...
For more details, see [Styling components](/react/guide/styling-components).
...
<img className="sbdocs-img" src={asset0} alt="Example of a brand ramp" />
