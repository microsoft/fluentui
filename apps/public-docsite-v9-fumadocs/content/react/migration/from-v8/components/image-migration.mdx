...
import { FitDifferences } from '../../../../../src/examples/image/FitDifferences.example';
...
| Image                          | v8 `imageFit="center"` with `border-radius`                                                | v9 `fit="center"` with `shape="circular"`                                                     |
| ------------------------------ | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------- |
| 100×100 (smaller than the box) | The image is centred and the radius clips the container, so the visible edge stays square. | `object-fit` leaves the image at its natural size, so the circular shape does not enclose it. |
| 500×500 (larger than the box)  | The image is centred and cropped by the container, producing a circle.                     | `object-fit: none` crops to the box and the circular shape encloses it.                       |

Use `fit="cover"` when the image has to fill the box for the shape to read as a circle.
