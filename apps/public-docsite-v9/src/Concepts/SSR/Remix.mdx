...
> **Component CSS needs no build plugin.** Fluent's component styles are precompiled — import each
> package's stylesheet (and `@fluentui/react-tailwind-theme/styles.css`) once at your app root. The
> `@griffel/vite-plugin` step below only optimizes `makeStyles` rules your own application authors;
> skip it if you have none. See
