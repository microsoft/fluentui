...
> **Component CSS needs no SSR wiring.** Import each package's precompiled stylesheet (and
> `@fluentui/react-tailwind-theme/styles.css`) once at your app root — Next.js handles it like any
> other CSS import. The renderer plumbing below remains for the theme's `<style>` tag and for any
> `makeStyles` rules your own application authors. See
