...
FluentUI will not be providing polyfills for features we expect our full browser support matrix (see below) to already support.
The only instance that a polyfill may be provided is when FluentUI's use of a feature causes an application to crash on
the partial support browser matrix (also below). Other than that, consumers will need to implement their own polyfills or use a third party one for their
application. Handling polyfills on the application level ensures that those on modern browsers don't gain unnecessary overhead of
