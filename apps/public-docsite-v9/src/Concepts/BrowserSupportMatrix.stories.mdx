...
FluentUI fully supports browsers that are at or above the indicated versions below. Consumers who support these browsers
are poised to experience the full performance and bundlesize benefits that the FluentUI library provides.
...
FluentUI introduces the concept of a partial browser support matrix as a means to find that balance between supporting modern browsers and older browsers.
The partial browser matrix below will have limitations that will require consumers to transpile down and/or use polyfills to cover the support gaps. The feature gaps
...
- Support for the CSS features below are lost within the partial support matrix. The lost of these features will not crash consumer applications,
...
- ES2020 features listed below are already being used within the FluentUI library. Consumers who follow the partial support matrix will need to transpile down
  the FluentUI library to make it work since the use of these ES2020 features will result in the application crashing on some of the listed browsers.
