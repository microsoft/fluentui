...
so this means that consumers who need to support older browsers (such as those that only support up to ES6) are responsible for transpiling down the FluentUI
library to fit their needs. This ensures that those consumers who support modern browsers can get the added benefit of a reduced bundle size that the latest ES standards provide.
