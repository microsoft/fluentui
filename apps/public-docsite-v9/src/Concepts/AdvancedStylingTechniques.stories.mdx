...
> ⚠️ Custom style hooks must also append the slot's original className prop as returned by `getSlotOriginalClassNameProp_unstable`, after the custom styles. This ensures that the className prop added by the user will take precedence over custom styles.

```ts
import { getSlotOriginalClassNameProp_unstable, makeStyles, type ButtonState } from '@fluentui/react-components';
...
export const useFancyButtonStyles = (state: unknown) => {
  ...
  buttonState.root.className = mergeClasses(
    ...
    styles.root,
    getSlotOriginalClassNameProp_unstable(buttonState.root),
  );
  ...
  if (buttonState.icon) {
    buttonState.icon.className = mergeClasses(
      ...
      styles.icon,
      getSlotOriginalClassNameProp_unstable(buttonState.icon),
    );
  }
};
```
