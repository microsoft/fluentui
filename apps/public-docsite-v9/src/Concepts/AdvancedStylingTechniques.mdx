...
> This page focuses on style customization of existing Fluent components. If you need architecture-level control over rendering and want Fluent behavior primitives without default styling, see [Base state hooks](?path=/docs/concepts-developer-base-state-hooks--docs).
