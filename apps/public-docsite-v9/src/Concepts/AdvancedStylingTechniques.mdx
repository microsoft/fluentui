...
export const useSmancyCustomButtonStyles = (state: unknown) => {
  let buttonState = state as ButtonState;

  // "Fancy" comes first
  buttonState = useFancyButtonStyles(buttonState);
  // "Smart" comes second, so it will win where there are conflicts
  buttonState = useSmartButtonStyles(buttonState);

  // A custom style hook returns the state it wants rendered — see the note above.
  return buttonState;
};
...
export function App() {
  return (
    <FluentProvider themeClassName={webLightThemeClassName}>
      <CustomStyleHooksProvider_unstable value={SMANCY_CUSTOM_STYLE_HOOKS}>
        {/* application code ... */}
      </CustomStyleHooksProvider_unstable>
    </FluentProvider>
  );
