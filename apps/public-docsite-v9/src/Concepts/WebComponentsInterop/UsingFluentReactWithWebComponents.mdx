...
The CSS custom properties just need to be defined in such a way that they will pierce the shadow DOM
(e.g., on the `:root` selector — custom properties inherit across shadow boundaries).

If your document already imports `@fluentui/react-tailwind-theme/styles.css`, the Web Light values are
emitted at `:root, :host` and inherit into shadow roots with no extra work. For a different theme, build
the rule from a theme object — theme objects (`webLightTheme`, …) ship from `@fluentui/tokens` as
build-time/tooling input, and the contrib package converts them:
...
# Style ordering inside a shadow root

Fluent's component CSS is a static stylesheet whose rules all live inside the `@layer fui.*` family.
That makes ordering a non-issue in the light DOM: your own unlayered rules — application utility
classes included — beat every Fluent rule regardless of document order, because the cascade compares
layers before it looks at specificity or position.

Inside a shadow root the same rule applies, with one thing you have to arrange yourself: static
stylesheets do not cross a shadow boundary, so both Fluent's component CSS and your own must be
adopted into the root explicitly.
...
// Your own rules. Unlayered, so they win over Fluent's layered component rules
// no matter which sheet is adopted first.
const appStyles = new CSSStyleSheet();
appStyles.insertRule('.my-style-from-outside-fluent { color: red; }');

<ThemelessFluentProvider>
  <root.div styleSheets={[appStyles]}>
    <Button className="my-style-from-outside-fluent">Button with external style</Button>
  </root.div>
</ThemelessFluentProvider>;
```

> ⚠ Delivering Fluent's own component stylesheet into a shadow root is not yet handled for you.
> Adopt the package stylesheets you need into the root alongside `appStyles` — otherwise the
> components inside it render unstyled.
