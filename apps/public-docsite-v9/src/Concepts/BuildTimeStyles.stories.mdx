...
The Webpack loader [evaluate styles at runtime using Linaria](#build-time-style-evaluation). For typescript
...
The Babel preset will [evaluate stlyes at runtime using Linaria](#build-time-style-evaluation). For typescript
...
It's perfectly fine, and recommended even to reuse common tokens and create style helpers across an application. It's
...
The build-time evaluation happens as a part of the Griffel Babel preset. All styles that require evaluation will be
batched and done in single evaluation context.**Linaria** Babel config separate to any config used by the application.
Therefore any additional language features
might require extra configuration. How to do this configuration is described in the sections above. One common
