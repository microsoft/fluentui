...
applications might want to optimize for performance. Generally, the expensive runtime work only
...
In your webpack config, simply add the `@griffel/webpack-loader` to the list of modules.
...
The babel preset is used internally by the webpack loader. For most applications the webpack loader should be enough.
...
module source. By default, the preset handles imports from `@fluentui/react-components`.
...
which describes what work is done during style resolution.

<img src={imageFile} alt={'Illustration of work done by style resolution'} />;

The final result before the CSS rules are inserted into DOM can be done during build time through
