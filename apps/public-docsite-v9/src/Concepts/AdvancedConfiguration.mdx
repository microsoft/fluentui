...
function MyComponent(props) {
  ...
  return <FluentProvider nonce="random">{children}</FluentProvider>;
}
```

Set it once on the app-root provider — nested `FluentProvider`s inherit the nonce for their own
theme style tags (and any of them can override it with its own `nonce` prop). The nonce is also
applied to the style element emitted during SSR. Component CSS is a static stylesheet
(`.css` assets), which a CSP covers via `style-src` source lists rather than nonces.

> **Migrating from an older setup?** The Griffel-era
> `RendererProvider` + `createDOMRenderer(document, { styleElementAttributes: { nonce } })`
> wrapper is no longer how the nonce reaches the theme style tag — replace it with the `nonce`
> prop above.
