...
Component CSS is a static stylesheet, so it does not travel with the React tree — load it into the
child document the same way you load it into the main one. What still needs configuring is the
`targetDocument`, so components resolve the right `window`/`document`, and the renderer that emits
the theme's custom-property `<style>` tag:
...
This applies to rules the [Griffel](https://griffel.js.org) runtime injects, which
[are sorted](https://griffel.js.org/react/guides/atomic-css#lvfha-order-of-pseudo-classes) for
deterministic behavior — the same applies to media queries. Fluent UI does not provide an
opinionated default to sort media queries as the order may vary depending on the specific needs of
the application.

> Fluent's own component styles are precompiled and are not injected by this renderer, so their
> media-query order is fixed at build time and unaffected by this setting. It applies to `makeStyles`
> rules your application still authors.
