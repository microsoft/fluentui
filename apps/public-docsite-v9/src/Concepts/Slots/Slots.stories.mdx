...
Fluent UI React components have parts that are designed to be replaced.
These are called slots.
Slots provide a more flexible approach over render callbacks used in previous versions.
...
- You can fill a slot with a primative value, JSX/TSX, props objects, or a render function.
...
- If you need to slightly adjust the display of a specific component instance, prefer to apply a custom style.
  For example, if you want to make a particular components have a thicker border.
...
You can pass a primative value to slots.
...
The typings are pretty dense and can be difficult to unwind,
...
and state for rendering using `ComponentState<Slot>`. The typings for these are complex, but end up being a set of slot definitions.
...
Hooks like `useButtonStyles` will typically create classes using `makeStyles` and `mergeClasses` and then
apply them conditionally based on the input state.
...
The `getSlots` method splits up the state to return back the slot elements to render and the props to apply.
...
- The slots API provides a powerful extensibility mechanism with a minimal surface area.
  While the typings are complex, they guide the caller with strong type safety.
