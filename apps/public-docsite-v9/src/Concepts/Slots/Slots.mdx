...
<table>
  ...
  <tbody>
    <tr>
      <td>`Slot<'div'>`</td>
      <td>A `div` is always rendered</td>
    </tr>
    <tr>
      <td>`Slot<typeof Button>`</td>
      <td>A `Button` component accepting `Button` props is rendered</td>
    </tr>
    <tr>
      <td>`Slot<'span', 'div' `&#124;` 'pre'>`</td>
      <td>A `span` is rendered by default. A caller can opt to render `div` or `pre` by using the `as` prop</td>
    </tr>
  </tbody>
</table>
