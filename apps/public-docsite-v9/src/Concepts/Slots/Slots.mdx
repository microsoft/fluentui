...
- If you need to slightly adjust the display of a specific component instance, prefer to apply a custom style.
  ...
  write a rule in your own stylesheet and apply it to the component using `className`.
...
```tsx
<>
  <Button icon={<img className="sbdocs-img" src='site-icon.png' alt='branded site icon' /> }
...
<button className="fui-Button">
  ...
  <span className="fui-Button__icon">
    <!-- icon slot children -->
    <img className="sbdocs-img" src="site-icon.png" alt="branded site icon" />
  </span>
</button>
```
...
```css
/* BusyBrandAvatar.module.css */
.badge {
  color: var(--color-brand-stroke-1);
}
```

```tsx
import styles from './BusyBrandAvatar.module.css';
...
Hooks like `useButtonStyles` conditionally apply the component's CSS-Module class names based on the input state, joining them with `clsx` and returning the new state.
