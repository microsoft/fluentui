...
Fluent UI v8 provides the `TextField` control to allow users to enter and edit text. Fluen UI v9 provides a `Textarea` control, but has a different API.

v9 has takes a different approach on the `TextField` component and uses the `Input` and `Textarea` components. `Input` in this case is the general `TextField` component and `Textarea` is a `TextField` with the `multiline` prop set to `true`.
...
const TextareaV9BasicExample = () => {
  ...
  return (
    <>
      ...
      <div>
        <Label disabled defaultValue={dummyText} htmlFor={disabledTextareaId}>
          Disabled
        </Label>
        <Textarea disabled id={disabledTextareaId} />
      </div>
      <div>
...
| `validateOnLoad`           | n/a            |                                                                                                                  |
| `value`                    | n/a            | Mutually exclusive with `defaultValue`                                                                           |
