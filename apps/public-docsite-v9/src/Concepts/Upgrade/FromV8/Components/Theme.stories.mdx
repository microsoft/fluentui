...
<Meta title="Concepts/Upgrading/from v8/Components/Theme Upgrade" />
...
ThemeProvider set theme values on the React context for use in CSS-inJS.
...
In v9, themes are seededwith `BrandVariants` along with the set of global colors.
...
### CSS-inJS style objects -> makeStyles
...
This function take a v8 `IPalette` and returns a `BrandVariants` you can use to create a v9 theme.
