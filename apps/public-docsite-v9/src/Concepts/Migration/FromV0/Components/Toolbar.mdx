...
@layer fui.components.l4 {
  .breakout-rooms-assignment-toolbar {
    width: 100%;
  }
}
```
...
export const Component = () => {
  return (
    <Toolbar className={styles['breakout-rooms-assignment-toolbar']}>
...
@layer fui.components.l4 {
  ...
  .live-tab-item-toolbar {
    color: var(--colorPaletteSilverForeground1);
  }
}
```
...
export const Component = props => {
  const { isLive } = props;
  return (
    <Toolbar className={clsx(styles['tab-item-toolbar'], isLive && styles['live-tab-item-toolbar'])}>
      <ToolbarButton>Italic</ToolbarButton>
    </Toolbar>
  );
};
