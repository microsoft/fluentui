...
> **Already on v9?** This page is about moving _to_ v9 from v8 or v0. If you are upgrading an
> existing v9 app across the major release that replaced Griffel with CSS Modules + Tailwind, go to
> [Upgrading from v9](?path=/docs/concepts-migration-upgrading-from-v9--docs) instead.
...
In v9, styles are created at build time to define class names. Components apply these class names at runtime based on state. The CSS variables those styles reference are declared by a static stylesheet: `@fluentui/react-tailwind-theme/styles.css` sets the defaults at the document root and ships one class per theme, so switching theme means applying a different class — no theme object is built or read at runtime.

Component styles are authored as Tailwind-flavored CSS Modules and compiled into a plain stylesheet
shipped with each package. There is no runtime style engine, so nothing has to be generated,
injected or de-duplicated while your app renders.
...
5. Import `@fluentui/react-tailwind-theme/styles.css` once per document. Wherever you are using `ThemeProvider`, add `FluentProvider` and set the `themeClassName` (e.g. `webLightThemeClassName`).
6. Build and test
