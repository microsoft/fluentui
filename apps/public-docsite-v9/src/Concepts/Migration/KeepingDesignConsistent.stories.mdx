...
Prefer creating custom themes, setting className, replacing slots, extendeding theme tokens, and component recomposition.
This approach is meant as an escape-hatch when all other customizations don't work.
...
You can pass `FluentProvider.customStyleHooks_unstable` a set of custom style hooks.
...
To override existing styles, use `makeStyles` to create your custom styles and then call `mergeClasses`.
To completely replace existing styles, overwrite `className`.
