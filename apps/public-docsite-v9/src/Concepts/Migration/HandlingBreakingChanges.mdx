...
In v9, styles are customized by writing ordinary CSS and setting the className prop.
...
To keep this customization in v9 Persona, write the rules in your own stylesheet and apply them to the associated slot.

```css
/* MyPersona.module.css */
.primary-text {
  color: steelblue;
}
...
```tsx
import personaStyles from './MyPersona.module.css';

//...

<Persona
  name="Anne Reid"
  primaryText={{ className: personaStyles['primary-text'] }}
  secondaryText={{ className: personaStyles['secondary-text'], children: 'Available' }}
  presence={{
    status: 'available',
  }}
/>;
