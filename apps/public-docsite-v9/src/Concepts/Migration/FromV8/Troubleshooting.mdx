...
- **Controlled state:** Keep editable text separate from committed parsed state
  when users must pass through temporarily invalid values. Define the commit
  boundary and resynchronize or revert invalid text on blur. Editing one
  channel must not reset unrelated state such as alpha.
