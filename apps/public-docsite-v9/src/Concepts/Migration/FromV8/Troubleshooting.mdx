...
In version 9, styling comes from CSS: import `@fluentui/react-tailwind-theme/styles.css` once per document. The stylesheet applies the Web Light theme by default; to use another theme, wrap your app with `FluentProvider` and pass a theme class to `themeClassName`.

We recommend placing the `FluentProvider` at the root of your app so that everything gets styled appropriately (including portals). The theme class to use if you are coming from version 8 is `webLightThemeClassName`.
...
We are also moving away from `mergeStyles`. There is no replacement API to learn: v9 components ship precompiled CSS inside the `@layer fui.*` family, so a rule you write in your own stylesheet already outranks the component's without any merging step, ordering trick or `!important`.
