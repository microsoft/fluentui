...
const getImportDeclarations = (sourceFile: ts.SourceFile): ts.ImportDeclaration[] =>
...
const getFencedCodeBlocks = (source: string): FencedCodeBlock[] => {
  const codeBlocks: FencedCodeBlock[] = [];
  let currentBlock: FencedCodeBlock | undefined;

  for (const [lineIndex, line] of stripHtmlComments(source).split(/\r?\n/).entries()) {
    const trimmedLine = line.trim();
...
const isV8MigrationImport = (moduleSpecifierText: string): boolean =>
  moduleSpecifierText === '@fluentui/react' || moduleSpecifierText.startsWith('@fluentui/react/lib/');
...
const validateCompleteGuide = (source: string, pageName: string): string[] => {
  const errors: string[] = [];
  const uncommentedSource = stripHtmlComments(source);
  ...
  let hasV8InlineExample = false;
  let hasV9InlineExample = false;

  for (const [index, codeBlock] of getFencedCodeBlocks(source).entries()) {
    ...
    for (const importDeclaration of getImportDeclarations(sourceFile)) {
      ...
      const { text: moduleSpecifierText } = importDeclaration.moduleSpecifier;

      hasV8InlineExample ||= isV8MigrationImport(moduleSpecifierText);
      hasV9InlineExample ||= moduleSpecifierText === '@fluentui/react-components';
    }
  }

  if (!hasV8InlineExample) {
    errors.push(`${pageName}: missing fenced tsx example importing from "@fluentui/react" or "@fluentui/react/lib/*".`);
  }

  if (!hasV9InlineExample) {
    errors.push(`${pageName}: missing fenced tsx example importing from "@fluentui/react-components".`);
  }

  const normalizedUncommentedSource = uncommentedSource.replace(/\s+/g, ' ');

  if (uncommentedSource.includes('<Canvas')) {
    errors.push(`${pageName}: must not use Storybook Canvas blocks; use inline fenced tsx examples instead.`);
  }

  if (/<Source\b[^>]*\bof\s*=/.test(normalizedUncommentedSource)) {
    errors.push(`${pageName}: must not use Storybook Source "of" references; use inline fenced tsx examples instead.`);
  }

  if (/import\s+\*\s+as\s+Examples\b/.test(uncommentedSource)) {
    errors.push(`${pageName}: must not import shared Examples stories.`);
  }

  if (uncommentedSource.includes('./examples/')) {
    errors.push(`${pageName}: must not reference "./examples/" files.`);
  }
...
`;
    ...
    expect(validateCompleteGuide(invalidGuide, 'Input')).toEqual([
      ...
      'Input: fenced tsx example 1 has syntax error at 33:55: Unterminated regular expression literal.',
      "Input: fenced tsx example 1 has syntax error at 33:57: '}' expected.",
      'Input: missing fenced tsx example importing from "@fluentui/react" or "@fluentui/react/lib/*".',
      'Input: missing fenced tsx example importing from "@fluentui/react-components".',
      'Input: must not use Storybook Canvas blocks; use inline fenced tsx examples instead.',
      'Input: must not use Storybook Source "of" references; use inline fenced tsx examples instead.',
      'Input: must not import shared Examples stories.',
      'Input: must not reference "./examples/" files.',
...
## Examples
### MC-1 v8 single-line field parity
\`\`\`tsx
import * as React from 'react';
import { TextField } from '@fluentui/react';

export const V8Example = () => <TextField label="Name" />;
\`\`\`

### MC-1 v9 single-line field parity
\`\`\`tsx
import * as React from 'react';
import { Field, Input } from '@fluentui/react-components';

export const V9Example = () => (
  <Field label="Name">
    <Input />
  </Field>
);
...
`;
  ...
  test('requires exact unsupported major-change ID matches', () => {
    const guideWithLooseIdMatch = `
...
`;
    ...
    expect(validateCompleteGuide(guideWithLooseIdMatch, 'Input')).toEqual([
...
`;
  ...
  test('requires the legacy shared examples directory to be removed', () => {
    expect(fs.existsSync(path.join(fromV8ComponentsDirectory, 'examples'))).toBe(false);
  });
