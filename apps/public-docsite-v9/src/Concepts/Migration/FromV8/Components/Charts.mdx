...
import type { Theme, BrandVariants } from '@fluentui/tokens';
...
export function ChartWrapperDocsite(props: { isDark?: boolean }) {
  // v9 themes are applied as CSS classes at runtime; the theme *objects* remain available
  // from @fluentui/tokens as build-time/tooling input. Pick the object that matches the
  // theme class your app applies (webLightTheme for webLightThemeClassName, etc.).
  let v9Theme: Theme = props.isDark ? webDarkTheme : webLightTheme;
