import { Meta } from '@storybook/addon-docs/blocks';

<Meta title="Concepts/Migration/from v8/Components/TextField to Input Migration" />

# TextField to Input Migration

## Overview

Fluent UI v8 provides the `TextField` control for entering and editing text. In Fluent UI v9 `TextField` is replaced with `Input` for single line input. For multiline input, use `Textarea`.

While basic usage is largely the same, `Input` omits some features found in `TextField`, preferring to compose several components together for greater flexibility.

## Examples

### Basic Migration

Basic usage of `TextField` looks like

```tsx
import * as React from 'react';
import { TextField } from '@fluentui/react/lib/TextField';
import { Stack, IStackProps, IStackStyles } from '@fluentui/react/lib/Stack';

const columnProps: Partial<IStackProps> = {
  tokens: { childrenGap: 15 },
  styles: { root: { width: 300 } },
};

const TextFieldV8BasicExample = () => {
  return (
    <Stack {...columnProps}>
      <TextField label="Standard" />
      <TextField label="Disabled" disabled defaultValue="I am disabled" />
      <TextField label="Read-only" readOnly defaultValue="I am read-only" />
      <TextField label="Required " required />
      <TextField ariaLabel="Required without visible label" required />
      <TextField label="With error message" errorMessage="Error message" />
    </Stack>
  );
};
```

### Validation

Validation is composed with `Field`, not unsupported. Move
`invalid`/`errorMessage` to `Field validationState` and
`validationMessage`. `Field` associates the message with the input, sets
`aria-invalid` for errors, and gives error/warning messages an alert role; do
not duplicate those ARIA attributes manually.

Keep the v8 validation policy in app state. The following preserves
change-driven async validation with an app-owned debounce and stale-result
protection:

```tsx
import * as React from 'react';
import { Field, Input } from '@fluentui/react-components';

const ValidatedInput = ({
  validate,
  validateOnLoad = false,
}: {
  validate: (value: string) => Promise<string>;
  validateOnLoad?: boolean;
}) => {
  const [value, setValue] = React.useState('');
  const [message, setMessage] = React.useState('');
  const [validationRequest, setValidationRequest] = React.useState(validateOnLoad ? 1 : 0);
  const requestSequence = React.useRef(0);

  React.useEffect(() => {
    if (validationRequest === 0) {
      return;
    }

    const sequence = ++requestSequence.current;
    const timer = window.setTimeout(async () => {
      const nextMessage = await validate(value);
      if (sequence === requestSequence.current) {
        setMessage(nextMessage);
      }
    }, 200);
    return () => window.clearTimeout(timer);
  }, [validate, validationRequest, value]);

  return (
    <Field label="Account name" validationState={message ? 'error' : 'none'} validationMessage={message || undefined}>
      <Input
        value={value}
        onChange={(_, data) => {
          setValue(data.value);
          setValidationRequest(request => request + 1);
        }}
      />
    </Field>
  );
};
```

When migrating `validateOnFocusIn` or `validateOnFocusOut`, run the same
sequenced validator from `onFocus` or `onBlur` instead of every change. If both
were true, validate on both events. Preserve `validateOnLoad` by either running
the validator on mount or skipping the initial effect/event until the first
requested validation. `deferredValidationTime` becomes the app-owned debounce.
Keep the last-request-wins guard (or cancellation) for promise validators.
Only update `validationMessage` when a result is current so stale errors are
not announced. Prefer an explicit validation-request counter or event-driven
validator over a one-time mount ref: React Strict Mode can replay mount effects,
which makes a ref latch accidentally validate despite `validateOnLoad={false}`.

An equivalent `Input` usage is:

```tsx
import * as React from 'react';
import { Field, makeStyles, Text, Label, Input } from '@fluentui/react-components';
import { useId } from '@fluentui/react-utilities';

const useStackStyles = makeStyles({
  root: {
    maxWidth: '300px',
    display: 'flex',
    flexDirection: 'column',

    '> * + *': {
      marginTop: '20px',
    },

    '> div': {
      display: 'flex',
      flexDirection: 'column',
    },
  },
});

const InputV9BasicExample = () => {
  const stackStyles = useStackStyles();
  const standardId = useId('standard');
  const disabledId = useId('disabled');
  const readonlyId = useId('readonly');
  const requiredId = useId('required');

  return (
    <div className={stackStyles.root}>
      <div>
        <Label htmlFor={standardId}>Standard</Label>
        <Input id={standardId} />
      </div>

      <div>
        <Label htmlFor={disabledId}>Disabled</Label>
        <Input id={disabledId} disabled defaultValue="I am disabled" />
      </div>

      <div>
        <Label htmlFor={readonlyId}>Read Only</Label>
        <Input id={readonlyId} readOnly defaultValue="I am read only" />
      </div>

      <div>
        <Label htmlFor={requiredId} required>
          Required
        </Label>
        <Input id={requiredId} required defaultValue="I am required" />
      </div>

      <div>
        <Input
          aria-label="Required without visible label"
          required
          defaultValue="I am required without a visible label"
        />
      </div>

      <Field label="With Error Message" validationState="error" validationMessage="Error message">
        <Input />
      </Field>

      <Field label="Required field" required>
        <Input required />
      </Field>
    </div>
  );
};
```

## Prop Mapping

This table maps v8 `TextField` props to the v9 `Input` equivalent.

| v8                         | v9                                         | Notes                                                                       |
| -------------------------- | ------------------------------------------ | --------------------------------------------------------------------------- |
| `componentRef`             | `ref`                                      | v9 provides access to the underlying DOM node, not `ITextField`             |
| `elementRef`               | `ref`                                      |                                                                             |
| `multiline`                | n/a                                        | Use `Textarea`                                                              |
| `resizable`                | n/a                                        |                                                                             |
| `autoAdjustHeight`         | n/a                                        | See `Textarea` docs                                                         |
| `underlined`               | `appearance="underline"`                   |                                                                             |
| `borderless`               | Custom `className` + `makeStyles`          | No v9 appearance is borderless; remove the root border with custom styling  |
| `label`                    | Use `Label` component                      | Be sure to associate `Label` with `Input` via `htmlFor`                     |
| `onRenderLabel`            | n/a                                        | Use slots to customize `Input`                                              |
| `description`              | `Field hint`                               | Use the `hint` slot to preserve field layout and the accessible description |
| `onRenderDescription`      | n/a                                        |                                                                             |
| `onRenderInput`            | n/a                                        | Use slots to customize `Input`                                              |
| `prefix`                   | `contentBefore`                            | This is a slot not a `string`                                               |
| `suffix`                   | `contentAfter`                             | This is a slot not a `string`                                               |
| `onRenderPrefix`           | n/a                                        | Use slots to customize `Input`                                              |
| `onRenderSuffix`           | n/a                                        | Use slots to customize `Input`                                              |
| `iconProps`                | Use `contentBefore` or `contentAfter` slot |                                                                             |
| `defaultValue`             | `defaultValue`                             | Mutually exclusive with `value`                                             |
| `value`                    | `value`                                    | Mutually exclusive with `defaultValue`                                      |
| `disabled`                 | `disabled`                                 |                                                                             |
| `readOnly`                 | `readOnly`                                 | In v9 this is the native HTML prop                                          |
| `required`                 | `required` on `Field` and `Input`          | Field renders the indicator; Input preserves native constraint validation   |
| `invalid`                  | `Field validationState="error"`            | `Field` supplies invalid semantics                                          |
| `errorMessage`             | `Field validationMessage`                  | `Field` associates and announces the message                                |
| `onChange`                 | `onChange`                                 | Typescript types have changed                                               |
| `onNotifyValidationResult` | App-owned validation state                 | Notify when the current validation result is applied                        |
| `onGetErrorMessage`        | App-owned validator + `Field`              | Preserve sync/async result and stale-result protection                      |
| `deferredValidationTime`   | App-owned debounce                         | Preserve the v8 delay before validation                                     |
| `className`                | `className`                                |                                                                             |
| `inputClassName`           | Use `input` slot                           |                                                                             |
| `ariaLabel`                | `aria-label`                               |                                                                             |
| `validateOnFocusIn`        | Validate from `onFocus`                    | Preserve the v8 timing policy                                               |
| `validateOnFocusOut`       | Validate from `onBlur`                     | Preserve the v8 timing policy                                               |
| `validateOnLoad`           | Initial app-owned validation               | Run or skip validation on mount to match v8                                 |
| `theme`                    | n/a                                        | Use `FluentProvider` to customize themes                                    |
| `styles`                   | `className`                                |                                                                             |
| `autoComplete`             | `autoComplete`                             | In v9 this is the native HTML prop                                          |
| `canRevealPassword`        | n/a                                        | v9 `Input` does not provide built in password reveal behavior               |
| `revealPasswordAriaLabel`  | n/a                                        | v9 `Input` does not provide built in password reveal behavior               |
