...
Use `ToolbarToggleButton` for checked command items. Wrap a
`ToolbarButton` in `MenuTrigger` when a v8 item has `subMenuProps`; render the
submenu entries as `MenuItem` children.
