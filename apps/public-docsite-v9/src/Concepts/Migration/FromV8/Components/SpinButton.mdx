...
const SpinButtonV9BasicExample = () => {
  ...
  return (
    <div className={layoutStyles.root}>
      ...
      <SpinButton
        className={layoutStyles.control}
        id={spinButtonId}
        value={value}
        min={0}
        max={100}
  ...
  return (
    <div className={layoutStyles.root}>
      ...
      <SpinButton
        ...
        max={100}
        onChange={onChange}
        incrementButton={{ 'aria-label': 'Increase value by 2' }}
        decrementButton={{ 'aria-label': 'Decrease value by 2' }}
      />
    </div>
  );
};
...
| `labelPosition`            | Use `Label` + CSS                         | v8 defaults to `Position.start`; place the label before the control in a row layout                      |
| `incrementButtonAriaLabel` | `incrementButton={{ 'aria-label': '…' }}` | Set the label through the increment button slot                                                          |
| `decrementButtonAriaLabel` | `decrementButton={{ 'aria-label': '…' }}` | Set the label through the decrement button slot                                                          |
