...
For v9, this feature is no longer supported. The alternative is to apply the animation through a CSS Module class and the global event `onLoad`.
...
const Component = () => {
  ...
  return (
    <Image
      ...
      className={!isLoaded ? styles['fade-in'] : ''}
    />
  );
};
```
...
For v9, this feature is no longer supported. The alternative is to apply styles through a CSS Module and `className`. Below is an example of a migration:
...
@layer fui.components.l4 {
  .bordered {
    border: var(--stroke-width-thin) solid var(--color-neutral-stroke-2);
  }
}
```
...
| \`shouldFadeIn\`         | Not supported -> implement animation via a CSS Module class + onLoad |
