...
Use the `dismiss` slot to preserve the explicit clear-button callback.
The dismiss slot runs before v9 reports the empty value through `onChange`, so
the controlled handler must honor `defaultPrevented` to preserve cancellation.
Do not infer a clear action from `onChange` receiving an empty value, because
ordinary typing also produces an empty value when the user deletes the final
character.
...
| `onClear`            | `dismiss.onClick` + Escape handler | Preserve explicit clear actions; do not infer from an empty `onChange` value                 |
