...
<MessageBar intent="error" layout="singleline">
  ...
  <MessageBarActions
    containerAction={
      <Button appearance="transparent" icon={<DismissRegular />} onClick={() => setVisible(false)} aria-label="Close" />
    }
  >
...
V8 defaults `isMultiline` to `true`. When the v8 prop is omitted, use
`layout="multiline"` explicitly to preserve its action placement and wrapping.
V9's default `layout="auto"` is responsive behavior, not the v8 multiline
default.
...
| `MessageBarType.warning`       | `"warning"` | Set `politeness="polite"` to preserve v8 announcement behavior |
| `MessageBarType.severeWarning` | `"warning"` | Keep the v9 default assertive behavior                         |
...
| `isMultiline`             | `layout`                             | Omitted/`true` → `"multiline"`; `false` → `"singleline"` |
