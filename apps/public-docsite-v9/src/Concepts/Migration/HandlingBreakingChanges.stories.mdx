...
Shim components depend on both v9 and the previous version, so you may not be able to reduce a bundle size increase.

We author shim components for popular component in the `@fluentui/react-migration-v8-v9` package.
...
Continuing the contextual menu button example: To have a contextual menu in v9, you can need to declare a `Menu` structure with `MenuItem` children.
...
For example, a v8 Persona with customized styles to display the primary text be steel blue and the secondary text have extra top and left margin.
