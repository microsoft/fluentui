...
In v9, you pass the `FluentProvider` component a theme class name (`themeClassName`).
Themes ship as static CSS classes that define a CSS variable for each design token.
...
Both `ThemeProvider` and `FluentProvider` support nesting to define a theme at different scopes — a nested `FluentProvider` inherits the parent's theme class when `themeClassName` is omitted.

```tsx
import { ThemeProvider, Button as ButtonV8 } from '@fluentui/react';
import { FluentProvider, Button as ButtonV9, webLightThemeClassName } from '@fluentui/react-components';
...
Fluent UI React v9 ships plain, precompiled CSS, so styling it needs nothing beyond whatever your
application already uses — a stylesheet, CSS Modules, Tailwind.
Read [Styling Components](/docs/concepts-developer-styling-components--docs) for details.

Because v9 theme classes define design token values as CSS variables, you can reference them in your own component styles.
Read [Theming](/docs/concepts-developer-theming--docs) to see examples.

## You can extend the theme with new design tokens

v9 theme classes define CSS variables consumed by components.
You can declare additional custom properties in your own CSS (for example in a custom theme class) and consume them in your own components.
...
Conversely, authoring a custom v9 theme class that uses v8 colors (a CSS class of custom-property declarations passed to `FluentProvider`'s `themeClassName`) will make v9 look more like v8.

### You can use theme shims

We have developed some shims (code that helps with migration) that let you create a v9 theme object from the default v8 theme, or create a v8 theme from the `webLightTheme` or `webDarkTheme` v9 theme objects (available from `@fluentui/tokens`). The resulting v9 theme object is build-time input — convert it to a custom CSS theme class to apply it.
...
As detailed in the _/Concepts/Developer/Styling Components_ topic, you can write your rules in your own stylesheet and pass the class name to any v9 component or component slot. Your rules win because they are unlayered and the component's are inside the `@layer fui.*` family — the cascade compares layers before it looks at specificity, so no `!important` or ordering trick is needed.
...
// This is an example of a custom style hook. It returns the state it wants rendered
// rather than mutating the state it was given.
const useCustomButtonStyles = (state: ButtonState): ButtonState => ({
  ...state,
  root: { ...state.root, className: styles.root },
});

export const MyButton: ForwardRefComponent<ButtonProps> = React.forwardRef((props, ref) => {
  let state = useButton(props, ref);
  state = useCustomButtonStyles(state);
...
- Return the state you want rendered; do not mutate the state you were handed.
...
import myButtonStyles from './MyButton.module.css';
...
const myCustomStyles: FluentProviderCustomStyleHooks = {
  useButtonStyles_unstable: state => {
    ...
    return {
      ...buttonState,
      root: { ...buttonState.root, className: clsx(buttonState.root.className, myButtonStyles.root) },
    };
  },
};
...
To add to the existing styles, `clsx` your class onto the state's `className`. To replace them
entirely, set `className` to your class alone. Either way your rule wins on the properties it sets,
because it is unlayered and the component's rules are inside `@layer fui.*`.

> Hooks written against the older `(state) => void` shape — which mutated the state object — keep
> working; the component normalizes an `undefined` return back to the state it passed in. New hooks
> should return.
