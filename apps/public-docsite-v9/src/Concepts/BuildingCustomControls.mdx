...
- Style logic (CSS-Module class composition, design token styling)
...
import { assertSlots, slot, type Slot } from '@fluentui/react-components';
import { useButtonBase_unstable } from '@fluentui/react-button';
import type { ButtonBaseProps, ButtonBaseState } from '@fluentui/react-button';
...
// --- Styles hook ---
// Join class names with clsx and RETURN the new state — never mutate the state you were handed.
// Put the existing slot className last by convention; which rule wins is decided by the cascade
// (your rules are unlayered or in fui.components.l3-l5, so they already beat the library's).

const useLoadingButtonStyles = (state: LoadingButtonState): LoadingButtonState => ({
  ...state,
  root: {
    ...state.root,
    className: clsx('loadingButton', state.isLoading && 'loadingButton--busy', state.root.className),
  },
  ...(state.loadingIndicator && {
    loadingIndicator: {
      ...state.loadingIndicator,
      className: clsx('loadingButton__indicator', state.loadingIndicator.className),
    },
  }),
});
...
export const LoadingButton = React.forwardRef<HTMLButtonElement | HTMLAnchorElement, LoadingButtonProps>(
  (props, ref) => {
    let state = useLoadingButtonState(props, ref);
    state = useLoadingButtonStyles(state);
...
- Keep the user's `className` on the element by passing existing slot class names as the last argument to `clsx`. Whether it _wins_ is a cascade question, not an argument-order one: unlayered rules beat every `fui.*` layer.
- Return a new state object from the styles hook rather than mutating the state you were passed.
