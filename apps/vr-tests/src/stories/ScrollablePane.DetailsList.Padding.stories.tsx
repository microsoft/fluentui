...
const paddingClass = mergeStyles({
  paddingLeft: '100px'
});
...
class ScrollablePaneDetailsListPaddingStory extends React.Component<{}, {}> {
...
function onRenderDetailsHeader(
  ...
  return (
    <Sticky stickyClassName={paddingClass} stickyPosition={StickyPositionType.Header} isScrollSynced={true}>
      {defaultRender!({
        ...props,
...
function onRenderDetailsFooter(props: IDetailsFooterProps): JSX.Element {
  return (
    <Sticky stickyClassName={paddingClass} stickyPosition={StickyPositionType.Footer} isScrollSynced={true}>
  ...
  .addDecorator(story => (
    <Screener
      steps={new Screener.Steps()
        ...
        .snapshot('scroll down to the bottom left', { cropTo: '.testWrapper' })
        .executeScript(
          "document.getElementsByClassName('ms-ScrollablePane--contentContainer')[0].scrollLeft = 99999"
        )
        .snapshot('scroll down to the bottom right', { cropTo: '.testWrapper' })
        .executeScript(
          "document.getElementsByClassName('ms-ScrollablePane--contentContainer')[0].scrollTop = 0"
        )
        .snapshot('scroll up to the top right', { cropTo: '.testWrapper' })
        .executeScript(
          "document.getElementsByClassName('ms-ScrollablePane--contentContainer')[0].scrollLeft = 0"
        )
        .snapshot('scroll up to the top left', { cropTo: '.testWrapper' })
        .end()}
    >
      {story()}
    </Screener>
  ))
  .addStory('ScrollablePane Details List custom padding', () => (
    <ScrollablePaneDetailsListPaddingStory />
  ));
