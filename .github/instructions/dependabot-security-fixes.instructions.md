...
1. **Security updates** - Advisory-driven pull requests created independently for each update
2. **npm dependencies** - Weekly minor and patch updates created as individual pull requests
3. **GitHub Actions** - Weekly updates for workflow dependencies created as individual pull requests
...
- **GitHub Actions**: Weekly updates as individual pull requests
- **Security updates**: Individual pull requests not limited by the version update schedule

The repository's Advanced Security **Grouped security updates** setting must remain disabled so security updates continue to be created as individual pull requests.

The npm `open-pull-requests-limit` controls the number of scheduled version-update pull requests. It does not change Dependabot's separate security-update pull request limit.

## Security Vulnerability Resolution

### Automatic Security Updates

GitHub triggers automatic security updates independently of the configured version update schedule. Each security update remains a separate pull request so compatibility work stays isolated for focused review.
