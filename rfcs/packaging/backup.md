...
- "Converged" component packages only contain convergence work
  ...
  - Open question: should we make code location or naming changes to any other `react-*` pacakges which still contain legacy components?
- Add badges (or similar) to package readmes to clarify GA/preview/dev status
- _(after v8 release)_ Add one or possibly two new suite packages which export _only_ converged components (`@fluentui/react-preview` and/or `@fluentui/react-components`)
...
|---------|-------|
| `@fluentui/react` | Exports official finished components which we plan to support long-term (or provide good migration paths for major changes). Mostly re-exports other packages, but a few components live here to avoid circular deps. |
| `@fluentui/react-internal` | Split out solely to work around circular dep issues, and most of the code for old components lives here. Nobody should reference it directly outside our repo. `@fluentui/react` re-exports everything from here, and some of the `@fluentui/react-<component>` packages depend on this package. |
...
🚨 See "open issues" below for more discussion. 🚨
