...
- Determinate Spinner
  - Add a `value` prop that prefills the `Spinner` to a specific percentage. Can be implemented using a value from 0 to 1.
...
```jsx
export type SpinnerProps = ComponentProps & React.HTMLAttributes<HTMLElement> & {
  ...
  // new prop for vertical? : boolean. Will allow for the labelPosition prop to only need to be "before" and "after"
  vertical?: boolean;
  ...
  /** The indeterminate prop sets whether the Spinner is a determinate Spinner or indetermindate Spinner*/
...
- **Display** - The Spinner will use the following priority:
  ...
  - The default indeterminate Spinner will be rendered with a label saying `Loading...` with the default color scheme
