...
In v9, components define slots that allow callers to replace a component part with their own. Slot props are like the previous callbacks but more powerful; they support literals, TSX/JSX, or render functions. See the [Customizing Components with Slots](./?path=/docs/concepts-developer-customizing-components-with-slots--docs) topic for more details.
...
Alternatively, you can make everything look like Fluent 1 or Fluent 2, but it takes some extra effort. We've provided some techniques and tools. See [Keeping Design Consistent](./?path=/docs/concepts-migration-keeping-design-consistent--docs).
