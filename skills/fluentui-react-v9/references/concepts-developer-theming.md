...
You can browse all the available tokens in **[Theme](./?path=/docs/theme-colors--docs)** section of the docs.
...
Place a `<FluentProvider />` at the root of your app and pass a theme to the `theme` prop. The provider will render a `div` and set all tokens as CSS variables on that element. The provider also propagates CSS variables to React portals created with [Portal component](./?path=/docs/components-portal-portal--docs).
