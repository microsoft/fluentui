...
export class Panel extends BaseComponent<IPanelProps, IPanelState> {
  ...
  public render() {
    let { children, className = '', type, hasCloseButton, isLightDismiss, headerText, closeButtonAriaLabel, hideHeaderText = false  } = this.props;
    ...
    let header;
    if (headerText) {
      header = <p className={ css('ms-Panel-headerText', {'ms-u-screenReaderOnly': hideHeaderText}) } id={ headerTextId }>{ headerText }</p>;
    }
