 ...
 * - request runtime dependencies of @fluentui/react-future package (by crawling the code, starting from index file),
...
const detectNonApprovedDependencies = async (dangerJS: DangerJS) => {
  ...
  const dependencyPackageIds = getRuntimeDependencies('react-future');
  const versionConstraints = await getVersionConstrains(paths.packages('react-future', 'package.json'));
