...
const makeConfig = (srcPath: string, name: string): webpack.Configuration => ({
  ...
  name: 'client',
  target: 'web',
  entry: paths.packageDist('react-future', path.join('es', srcPath)),
  output: {
    filename: `${name}.js`,
...
export default [
  ...
  ...fs.readdirSync(paths.packageSrc('react-future', 'components')).map(dir => makeConfig(`components/${dir}/${dir}`, `component-${dir}`)),

  // individual themes
  ...fs
    .readdirSync(paths.packageSrc('react-future', 'themes'))
