...
const tslintPath = path.join(path.dirname(require.resolve('tslint/package.json')), 'lib', 'tslintCli.js');
...
function runTsLintOnFilesGroupedPerPackage(filesGroupedByPackage) {
  ...
  for (let [package, files] of fileEntries) {
    ...
    execSync(`node ${tslintPath} --config ${tslintConfig} -t stylish -r ${rulesPath} ${filteredFiles.join(' ')}`);
  }
}
